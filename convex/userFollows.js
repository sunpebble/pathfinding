...
export const getMutualFollowStatus = query({
    ...
    handler: async (ctx, args) => {
        const aFollowsB = await ctx.db
            .query('userFollows')
            .withIndex('by_follower_following', (q) => q.eq('followerId', args.userIdA).eq('followingId', args.userIdB))
            .first();
        const bFollowsA = await ctx.db
            .query('userFollows')
            .withIndex('by_follower_following', (q) => q.eq('followerId', args.userIdB).eq('followingId', args.userIdA))
            .first();
        return {
...
export const getFollowers = query({
    ...
    handler: async (ctx, args) => {
        ...
        const enrichedFollowers = await Promise.all(paginatedFollows.map(async (follow) => {
            ...
            if (args.currentUserId && args.currentUserId !== follow.followerId) {
                const currentUserFollows = await ctx.db
                    .query('userFollows')
                    .withIndex('by_follower_following', (q) => q
...
export const getFollowing = query({
    ...
    handler: async (ctx, args) => {
        ...
        const paginatedFollows = follows.slice(offset, offset + pageSize);
        // Enrich with profile data and follow status
        const enrichedFollowing = await Promise.all(paginatedFollows.map(async (follow) => {
            const profile = await ctx.db
                .query('profiles')
...
export const getFollowingFeed = query({
    ...
    handler: async (ctx, args) => {
        ...
        const feedItineraries = allItineraries.filter((itinerary) => followingIds.includes(itinerary.userId));
        const total = feedItineraries.length;
        const paginatedItineraries = feedItineraries.slice(offset, offset + pageSize);
        // Enrich with city and user data
        const enrichedItineraries = await Promise.all(paginatedItineraries.map(async (itinerary) => {
            const city = await ctx.db.get(itinerary.cityId);
            const userProfile = await ctx.db
                .query('profiles')
                .withIndex('by_email', (q) => q.eq('email', itinerary.userId))
                .first();
            const daysCount = calculateDaysCount(itinerary.startDate, itinerary.endDate);
            return {
                ...itinerary,
                cityName: city?.name,
                daysCount,
...
export const getMutualFollowers = query({
    ...
    handler: async (ctx, args) => {
        ...
        const mutualFollows = followers.filter((f) => followingIds.has(f.followerId));
        const total = mutualFollows.length;
        const paginatedMutuals = mutualFollows.slice(offset, offset + pageSize);
        // Enrich with profile data
        const enrichedMutuals = await Promise.all(paginatedMutuals.map(async (follow) => {
            const profile = await ctx.db
                .query('profiles')
                .withIndex('by_email', (q) => q.eq('email', follow.followerId))
                .first();
    ...
    handler: async (ctx, args) => {
        ...
        for (const activity of recentActivities) {
            if (!followingIds.has(activity.actorId)) {
                activeCreators.set(activity.actorId, (activeCreators.get(activity.actorId) ?? 0) + 1);
            }
        }
        ...
        const sortedRecommendations = Array.from(recommendations.entries())
            ...
            .slice(0, limit);
        // Enrich with profile data
        const enrichedRecommendations = await Promise.all(sortedRecommendations.map(async ([userId, data]) => {
            const profile = await ctx.db
                .query('profiles')
            ...
            const followsBack = await ctx.db
                .query('userFollows')
                .withIndex('by_follower_following', (q) => q.eq('followerId', userId).eq('followingId', args.userId))
                .first();
            return {
                id: userId,
                displayName: profile?.displayName,
    ...
    handler: async (ctx, args) => {
        ...
        for (const targetId of args.targetUserIds) {
            const isFollowing = await ctx.db
                .query('userFollows')
                .withIndex('by_follower_following', (q) => q.eq('followerId', args.userId).eq('followingId', targetId))
                .first();
            const isFollowedBy = await ctx.db
                .query('userFollows')
                .withIndex('by_follower_following', (q) => q.eq('followerId', targetId).eq('followingId', args.userId))
                .first();
            results[targetId] = {
