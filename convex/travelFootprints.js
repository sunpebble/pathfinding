...
export const addVisitedCity = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query('visitedCities')
            .withIndex('by_user_city', (q) => q.eq('userId', args.userId).eq('cityName', args.cityName))
            .first();
        if (existing) {
...
export const upsertVisitedCountry = mutation({
    ...
    handler: async (ctx, args) => {
        const existing = await ctx.db
            .query('visitedCountries')
            .withIndex('by_user_country', (q) => q.eq('userId', args.userId).eq('countryCode', args.countryCode))
            .first();
        if (existing) {
            // Update city count
            const cities = await ctx.db
                ...
                .withIndex('by_user', (q) => q.eq('userId', args.userId))
                .collect();
            const countryCities = cities.filter((c) => c.countryCode === args.countryCode);
            await ctx.db.patch(existing._id, {
                citiesCount: countryCities.length,
                lastVisitedAt: Math.max(...countryCities.map((c) => c.lastVisitedAt || c.visitedAt)),
            });
            return existing._id;
        }
...
export const updateTravelStats = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const mostVisitedCity = cities.reduce((max, city) => (city.visitCount || 1) > (max?.visitCount || 0) ? city : max, null);
        // Find most visited country (by cities count)
        const mostVisitedCountry = countries.reduce((max, country) => (country.citiesCount || 1) > (max?.citiesCount || 0) ? country : max, null);
        // Get date range
        ...
        for (const city of cities) {
            const year = new Date(city.firstVisitedAt || city.visitedAt)
                .getFullYear()
                .toString();
            if (!yearlyStats[year]) {
        ...
        const yearlyStatsFormatted = Object.fromEntries(Object.entries(yearlyStats).map(([year, data]) => [
            year,
        ...
        const statsData = {
            ...
            mostVisitedCity: mostVisitedCity
                ? {
                    name: mostVisitedCity.cityName,
                    count: mostVisitedCity.visitCount || 1,
                }
                : null,
            mostVisitedCountry: mostVisitedCountry
                ? {
                    name: mostVisitedCountry.countryName,
                    count: mostVisitedCountry.citiesCount || 1,
                }
                : null,
            firstTripDate,
            lastTripDate,
...
export const getTravelTimeline = query({
    ...
    handler: async (ctx, args) => {
        ...
        const sorted = allCities.sort((a, b) => (b.lastVisitedAt || b.visitedAt) - (a.lastVisitedAt || a.visitedAt));
