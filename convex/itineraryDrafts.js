...
const transportModeValidator = v.union(v.literal('walking'), v.literal('driving'), v.literal('transit'), v.literal('cycling'), v.literal('taxi'));
...
export const listByUser = query({
    ...
    handler: async (ctx, args) => {
        ...
        const enriched = await Promise.all(data.map(async (draft) => {
...
export const getByItinerary = query({
    ...
    handler: async (ctx, args) => {
        const draft = await ctx.db
            .query('itineraryDrafts')
            .withIndex('by_user_itinerary', (q) => q.eq('userId', args.userId).eq('itineraryId', args.itineraryId))
            .first();
    ...
    handler: async (ctx, args) => {
        ...
        if (args.draftId) {
            ...
            if (args.expectedVersion !== undefined &&
                ...
                throw new Error('Draft has been modified by another device. Please refresh and try again.');
            }
