...
export const getByUserAndDestination = query({
    ...
    handler: async (ctx, args) => {
        const checklists = await ctx.db
            ...
            .withIndex('by_user_destination', (q) => q.eq('userId', args.userId).eq('destinationId', args.destinationId))
            .collect();
...
export const create = mutation({
    args: {
        ...
        customItems: v.optional(v.array(v.object({
            name: v.string(),
            category: v.string(),
...
export const toggleCustomItem = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const currentItems = checklist.customItems ?? [];
        const updatedItems = currentItems.map((item) => item.name === args.itemName
...
export const removeCustomItem = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const currentItems = checklist.customItems ?? [];
        const updatedItems = currentItems.filter((item) => item.name !== args.itemName);
        await ctx.db.patch(args.id, {
            customItems: updatedItems,
            updatedAt: Date.now(),
        });
