...
const eventTypeValidator = v.union(v.literal('festival'), v.literal('concert'), v.literal('exhibition'), v.literal('sports'), v.literal('food'), v.literal('cultural'), v.literal('market'), v.literal('performance'), v.literal('religious'), v.literal('seasonal'), v.literal('local_custom'), v.literal('other'));
// Event status validator
const eventStatusValidator = v.union(v.literal('upcoming'), v.literal('ongoing'), v.literal('ended'), v.literal('cancelled'));
// ============================================
...
export const listUpcoming = query({
    ...
    handler: async (ctx, args) => {
        ...
        const events = await ctx.db
            .query('localEvents')
            .withIndex('by_city_status', (q) => q.eq('cityId', args.cityId).eq('status', 'upcoming'))
            .collect();
...
export const listOngoing = query({
    ...
    handler: async (ctx, args) => {
        ...
        const events = await ctx.db
            .query('localEvents')
            .withIndex('by_city_status', (q) => q.eq('cityId', args.cityId).eq('status', 'ongoing'))
            .take(limit);
        return events;
    },
});
...
export const listFeatured = query({
    ...
    handler: async (ctx, args) => {
        ...
        const featured = allEvents
            .filter((e) => e.isFeatured && e.status !== 'ended' && e.status !== 'cancelled')
            .slice(0, limit);
        return featured;
    },
});
...
export const listFavorites = query({
    ...
    handler: async (ctx, args) => {
        ...
        // Fetch event details
        const events = await Promise.all(paginated.map(async (fav) => {
...
export const createReminder = mutation({
    args: {
        userId: v.string(),
        eventId: v.id('localEvents'),
        reminderType: v.union(v.literal('event_start'), v.literal('booking_open'), v.literal('custom')),
...
export const listReminders = query({
    ...
    handler: async (ctx, args) => {
        ...
        // Fetch event details
        const withEvents = await Promise.all(filtered.map(async (reminder) => {
...
export const getPendingReminders = query({
    ...
    handler: async (ctx) => {
        ...
        // Fetch event details
        const withEvents = await Promise.all(pending.map(async (reminder) => {
            const event = await ctx.db.get(reminder.eventId);
            return { ...reminder, event };
        }));
        return withEvents;
