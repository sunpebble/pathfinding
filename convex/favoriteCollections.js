...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existingCollections = await ctx.db
            ...
            .collect();
        const maxSortOrder = existingCollections.reduce((max, c) => Math.max(max, c.sortOrder), 0);
...
export const getById = query({
    ...
    handler: async (ctx, args) => {
        ...
        const enrichedFavorites = await Promise.all(paginatedFavorites.map(async (fav) => {
    ...
    handler: async (ctx, args) => {
        ...
        if (collection.isDefault) {
            const favorites = await ctx.db
                ...
                .first();
            if (favorites) {
                throw new Error('Cannot delete default collection while it contains items');
            }
        }
