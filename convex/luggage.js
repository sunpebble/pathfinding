 ...
 */
// Luggage status enum
const luggageStatusValidator = v.union(v.literal('checked_in'), v.literal('in_transit'), v.literal('arrived'), v.literal('claimed'), v.literal('delayed'), v.literal('lost'), v.literal('found'), v.literal('damaged'));
// Luggage size enum
const luggageSizeValidator = v.union(v.literal('cabin'), v.literal('medium'), v.literal('large'), v.literal('oversized'));
/**
 * Permission checking helpers
 */
async function checkLuggageOwnership(ctx, luggageId, userId) {
...
export const list = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.flightBookingId) {
            luggageItems = luggageItems.filter((l) => l.flightBookingId === args.flightBookingId);
        }
        if (args.itineraryId) {
            luggageItems = luggageItems.filter((l) => l.itineraryId === args.itineraryId);
        }
...
export const getByTagNumber = query({
    ...
    handler: async (ctx, args) => {
        const luggage = await ctx.db
            .query('luggage')
            .withIndex('by_tag_number', (q) => q.eq('tagNumber', args.tagNumber.toUpperCase()))
            .first();
...
export const getByFlightBooking = query({
    ...
    handler: async (ctx, args) => {
        const luggageItems = await ctx.db
            .query('luggage')
            .withIndex('by_flight_booking', (q) => q.eq('flightBookingId', args.flightBookingId))
            .collect();
        // Filter by user
...
export const update = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (filteredUpdates.tagNumber) {
            filteredUpdates.tagNumber = filteredUpdates.tagNumber.toUpperCase();
        }
        if (filteredUpdates.airlineCode) {
            filteredUpdates.airlineCode = filteredUpdates.airlineCode.toUpperCase();
        }
...
export const getLossReportTemplate = query({
    ...
    handler: async (ctx, args) => {
        const template = await ctx.db
            ...
            .withIndex('by_airline_code', (q) => q.eq('airlineCode', args.airlineCode.toUpperCase()))
            .first();
        return template;
    },
});
