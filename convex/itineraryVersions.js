...
const visibilityValidator = v.union(v.literal('private'), v.literal('team'), v.literal('public'));
 ...
 */
async function checkVersionAccess(ctx, itineraryId, userId) {
    ...
    const collab = await ctx.db
        ...
        .withIndex('by_itinerary_user', (q) => q.eq('itineraryId', itineraryId).eq('userId', userId))
        .first();
    if (!collab) {
 ...
 */
async function getNextVersionNumber(ctx, itineraryId) {
 ...
 */
async function createItinerarySnapshot(ctx, itineraryId) {
    const itinerary = await ctx.db.get(itineraryId);
    if (!itinerary) {
        throw new Error('Itinerary not found');
    }
    ...
    const daysWithItems = await Promise.all(days.map(async (day) => {
 ...
 */
function calculateChanges(oldSnapshot, newSnapshot) {
    ...
    // Count item changes
    const oldItemsByDay = new Map();
    const newItemsByDay = new Map();
    for (const day of oldSnapshot.days) {
        oldItemsByDay.set(day.dayNumber, new Set(day.items.map((i) => `${i.poiId}:${i.orderIndex}`)));
    }
    for (const day of newSnapshot.days) {
        newItemsByDay.set(day.dayNumber, new Set(day.items.map((i) => `${i.poiId}:${i.orderIndex}`)));
    }
    ...
    handler: async (ctx, args) => {
        ...
        const enriched = data.map((version) => ({
            ...
            snapshotMeta: {
                ...
                itemsCount: version.snapshot.days.reduce((acc, day) => acc + day.items.length, 0),
            },
        }));
...
export const getById = query({
    ...
    handler: async (ctx, args) => {
        ...
        const enrichedDays = await Promise.all(version.snapshot.days.map(async (day) => {
    ...
    handler: async (ctx, args) => {
        ...
        if (versionNumber > 1) {
            const prevVersion = await ctx.db
                .query('itineraryVersions')
                .withIndex('by_itinerary_version', (q) => q
...
export const restore = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (createBackup) {
            const currentSnapshot = await createItinerarySnapshot(ctx, version.itineraryId);
            const backupVersionNumber = await getNextVersionNumber(ctx, version.itineraryId);
        ...
        const existingDays = await ctx.db
            .query('itineraryDays')
            .withIndex('by_itinerary', (q) => q.eq('itineraryId', version.itineraryId))
            .collect();
