...
const searchResultTypeValidator = v.union(v.literal('poi'), v.literal('itinerary'), v.literal('guide'), v.literal('user'));
/**
    ...
    handler: async (ctx, args) => {
        ...
        if (types.includes('guide')) {
            ...
            for (const guide of guides) {
                ...
                if (guide.destinations.some((d) => d.toLowerCase().includes(searchQuery))) {
    ...
    handler: async (ctx, args) => {
        ...
        for (const guide of guides) {
            for (const dest of guide.destinations) {
                if (dest.toLowerCase().startsWith(searchQuery) &&
...
export const recordSearchHistory = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query('searchHistory')
            .withIndex('by_user_query', (q) => q.eq('userId', args.userId).eq('query', args.query))
            .first();
        if (existing) {
            // Update timestamp
...
export const recordSearchTrend = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query('searchTrends')
            .withIndex('by_query_date', (q) => q.eq('query', normalizedQuery).eq('date', today))
            .first();
        if (existing) {
            await ctx.db.patch(existing._id, {
