 ...
 */
const photoStatusValidator = v.union(v.literal('pending'), v.literal('approved'), v.literal('rejected'), v.literal('hidden'));
const photoCategoryValidator = v.union(v.literal('interior'), v.literal('exterior'), v.literal('food'), v.literal('scenery'), v.literal('activity'), v.literal('detail'), v.literal('other'));
// ============================================
// Queries
// ============================================
...
export const listByPoi = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.category) {
            ...
            photosQuery = ctx.db
                .query('poiPhotos')
                .withIndex('by_poi_category', (q) => q.eq('poiId', args.poiId).eq('category', args.category))
            ...
            photosQuery = ctx.db
                .query('poiPhotos')
                .withIndex('by_poi_status', (q) => q.eq('poiId', args.poiId).eq('status', status))
                .order('desc');
        }
...
export const getFeaturedByPoi = query({
    ...
    handler: async (ctx, args) => {
        ...
        const photos = await ctx.db
            .query('poiPhotos')
            .withIndex('by_poi_featured', (q) => q.eq('poiId', args.poiId).eq('isFeatured', true))
            .order('desc')
            .take(limit);
        return photos;
    },
...
export const getPopularPhotos = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.poiId) {
            query = ctx.db
                .query('poiPhotos')
                .withIndex('by_poi_status', (q) => q.eq('poiId', args.poiId).eq('status', 'approved'));
        }
        else {
            query = ctx.db
                .query('poiPhotos')
...
export const getUserLikedPhotos = query({
    ...
    handler: async (ctx, args) => {
        ...
        const likes = await ctx.db
            ...
            .order('desc')
            .take(limit);
        const photos = await Promise.all(likes.map(async (like) => {
