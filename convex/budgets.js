...
export const getBudgetWithCategories = query({
    ...
    handler: async (ctx, args) => {
        ...
        const enrichedCategoryBudgets = await Promise.all(budget.categoryBudgets.map(async (cb) => {
    ...
    handler: async (ctx, args) => {
        ...
        if (args.categoryId) {
            expenses = await ctx.db
                .query('expenses')
                .withIndex('by_itinerary_category', (q) => q
...
export const getBudgetSummary = query({
    ...
    handler: async (ctx, args) => {
        ...
        const spendingByCategory = categories.map((category) => {
            ...
            const budgetAmount = budget?.categoryBudgets.find((cb) => cb.categoryId === category._id)
                ?.amount || 0;
            return {
                category,
                budgetAmount,
        ...
        return {
            ...
            percentUsed: budget && budget.totalBudget > 0
