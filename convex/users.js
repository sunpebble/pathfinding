    ...
    handler: async (ctx, args) => {
        ...
        if (args.currentUserId && args.currentUserId !== args.userId) {
            const currentUserFollows = await ctx.db
                .query('userFollows')
                .withIndex('by_follower_following', (q) => q.eq('followerId', args.currentUserId).eq('followingId', args.userId))
                .first();
            isFollowing = currentUserFollows !== null;
            const targetUserFollows = await ctx.db
                .query('userFollows')
                .withIndex('by_follower_following', (q) => q.eq('followerId', args.userId).eq('followingId', args.currentUserId))
                .first();
