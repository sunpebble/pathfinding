...
export const listByUser = query({
    ...
    handler: async (ctx, args) => {
        ...
        const favoritesWithDetails = await Promise.all(paginatedFavorites.map(async (fav) => {
...
export const isFavorited = query({
    ...
    handler: async (ctx, args) => {
        const favorites = await ctx.db
            .query('favoriteSimCards')
            .withIndex('by_user_sim_card', (q) => q.eq('userId', args.userId).eq('simCardId', args.simCardId))
            .collect();
