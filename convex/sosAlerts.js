...
export const getActiveAlerts = query({
    ...
    handler: async (ctx, args) => {
        const alerts = await ctx.db
            .query('sosAlerts')
            .withIndex('by_user_status', (q) => q.eq('userId', args.userId).eq('status', 'sent'))
            .collect();
        return alerts;
    },
});
...
export const getWithContacts = query({
    ...
    handler: async (ctx, args) => {
        ...
        const contacts = await Promise.all(alert.notifiedContacts.map(async (contactId) => {
...
export const getRecentAlerts = query({
    args: {
        ...
        status: v.optional(v.union(v.literal('sent'), v.literal('received'), v.literal('resolved'), v.literal('cancelled'))),
    },
    handler: async (ctx, args) => {
