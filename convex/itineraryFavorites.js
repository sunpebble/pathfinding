    ...
    handler: async (ctx, args) => {
        ...
        if (args.collectionId) {
            favorites = await ctx.db
                .query('itineraryFavorites')
                .withIndex('by_user_collection', (q) => q.eq('userId', args.userId).eq('collectionId', args.collectionId))
                .order('desc')
                .collect();
        }
        else {
        ...
        const enriched = await Promise.all(paginatedFavorites.map(async (fav) => {
            ...
            const collection = fav.collectionId
                ? (await ctx.db.get(fav.collectionId))
                : null;
            return {
                ...fav,
                itinerary: {
...
export const batchCheckFavorites = query({
    ...
    handler: async (ctx, args) => {
        const results = {};
        await Promise.all(args.itineraryIds.map(async (itineraryId) => {
            const favorite = await ctx.db
                .query('itineraryFavorites')
                .withIndex('by_user_itinerary', (q) => q.eq('userId', args.userId).eq('itineraryId', itineraryId))
                .first();
