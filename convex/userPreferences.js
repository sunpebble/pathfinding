...
const preferenceCategories = v.union(v.literal('food'), // Food & Dining
...
export const upsertPreferences = mutation({
    args: {
        ...
        travelStyle: v.optional(v.union(v.literal('adventurous'), v.literal('relaxed'), v.literal('cultural'), v.literal('balanced'))),
        budgetLevel: v.optional(v.union(v.literal('budget'), v.literal('moderate'), v.literal('luxury'))),
        pacePreference: v.optional(v.union(v.literal('slow'), // 1-2 activities per day
...
export const recordBehavior = mutation({
    args: {
        userId: v.string(),
        behaviorType: v.union(v.literal('view'), // Viewed content
    ...
    handler: async (ctx, args) => {
        ...
        if (args.categories && args.categories.length > 0) {
            ...
            if (preferences) {
                ...
                const currentScores = preferences.categoryScores || {};
...
export const getRecentBehaviors = query({
    args: {
        userId: v.string(),
        limit: v.optional(v.number()),
        behaviorType: v.optional(v.union(v.literal('view'), v.literal('save'), v.literal('unsave'), v.literal('copy'), v.literal('share'), v.literal('like'), v.literal('unlike'), v.literal('search'), v.literal('poi_click'), v.literal('poi_add'))),
    },
    handler: async (ctx, args) => {
        const query = ctx.db
