 ...
 */
const messageTypeValidator = v.union(v.literal('text'), v.literal('image'), v.literal('itinerary_share'));
// ============================================
...
export const listConversations = query({
    ...
    handler: async (ctx, args) => {
        ...
        const userConversations = allConversations.filter((conv) => conv.participantIds.includes(args.userId));
        const limitedConversations = userConversations.slice(0, limit);
        // Enrich with participant profiles and unread counts
        const enriched = await Promise.all(limitedConversations.map(async (conv) => {
            ...
            const readStatus = await ctx.db
                ...
                .withIndex('by_conversation_user', (q) => q.eq('conversationId', conv._id).eq('userId', args.userId))
                .first();
            let unreadCount = 0;
            if (readStatus?.lastReadAt) {
                ...
                const unreadMessages = await ctx.db
                    .query('messages')
                    .withIndex('by_conversation_time', (q) => q
                    .eq('conversationId', conv._id)
                    .gt('sentAt', readStatus.lastReadAt))
                    .collect();
                unreadCount = unreadMessages.filter((msg) => msg.senderId !== args.userId && !msg.isDeleted).length;
            }
                ...
                const allMessages = await ctx.db
                    ...
                    .collect();
                unreadCount = allMessages.filter((msg) => msg.senderId !== args.userId && !msg.isDeleted).length;
            }
            return {
                ...conv,
                otherUser,
...
export const getOrCreateConversation = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = allConversations.find((conv) => conv.participantIds.length === 2 &&
...
export const sendMessage = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const previewText = args.messageType === 'text'
        ...
        const senderReadStatus = await ctx.db
            .query('messageReadStatus')
            .withIndex('by_conversation_user', (q) => q.eq('conversationId', args.conversationId).eq('userId', args.senderId))
            .first();
        if (senderReadStatus) {
        ...
        const recipientId = conversation.participantIds.find((id) => id !== args.senderId);
        if (recipientId) {
...
export const markAsRead = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const readStatus = await ctx.db
            .query('messageReadStatus')
            .withIndex('by_conversation_user', (q) => q.eq('conversationId', args.conversationId).eq('userId', args.userId))
            .first();
        if (readStatus) {
    ...
    handler: async (ctx, args) => {
        ...
        for (const conv of userConversations) {
            ...
            if (readStatus?.lastReadAt) {
                const unreadMessages = await ctx.db
                    .query('messages')
                    .withIndex('by_conversation_time', (q) => q.eq('conversationId', conv._id).gt('sentAt', readStatus.lastReadAt))
                    .collect();
                totalUnread += unreadMessages.filter((msg) => msg.senderId !== args.userId && !msg.isDeleted).length;
            }
            else if (conv.lastMessageAt) {
                const allMessages = await ctx.db
                    .query('messages')
                    .withIndex('by_conversation', (q) => q.eq('conversationId', conv._id))
                    .collect();
                totalUnread += allMessages.filter((msg) => msg.senderId !== args.userId && !msg.isDeleted).length;
            }
        }
        return totalUnread;
    },
