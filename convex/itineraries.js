...
async function checkEditPermission(ctx, itineraryId, userId) {
...
async function checkOwnerPermission(ctx, itineraryId, userId) {
    const itinerary = await ctx.db.get(itineraryId);
    if (!itinerary) {
        throw new Error('Itinerary not found');
    }
...
export const listByUser = query({
    ...
    handler: async (ctx, args) => {
        ...
        const enriched = await Promise.all(data.map(async (itinerary) => {
...
export const getById = query({
    ...
    handler: async (ctx, args) => {
        ...
        const allItems = await Promise.all(days.map((day) => ctx.db
        ...
        const pois = await Promise.all(Array.from(poiIds).map((id) => ctx.db.get(id)));
...
export const copy = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const dateOffset = Math.floor((newStart.getTime() - originalStart.getTime()) / (1000 * 60 * 60 * 24));
...
export const getItineraryCopyStats = query({
    ...
    handler: async (ctx, args) => {
        const copies = await ctx.db
            .query('itineraryCopyHistory')
            .withIndex('by_original', (q) => q.eq('originalItineraryId', args.itineraryId))
            .collect();
        return {
