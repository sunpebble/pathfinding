...
export const search = query({
    ...
    handler: async (ctx, args) => {
        ...
        const filtered = credentials.filter((cred) => cred.name.toLowerCase().includes(searchLower) ||
...
export const getSharedBySpot = query({
    ...
    handler: async (ctx, args) => {
        const results = await ctx.db
            .query('wifiCredentials')
            .withIndex('by_spot_shared', (q) => q.eq('wifiSpotId', args.wifiSpotId).eq('isShared', true))
            .collect();
