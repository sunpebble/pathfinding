...
// Validators
const platformValidator = v.union(v.literal('wechat'), v.literal('weibo'), v.literal('xiaohongshu'), v.literal('qq'), v.literal('douyin'), v.literal('copy_link'), v.literal('system_share'), v.literal('generic'));
const eventTypeValidator = v.union(v.literal('share'), v.literal('click'), v.literal('view'), v.literal('save'));
const permissionValidator = v.union(v.literal('public'), // Anyone with link can view
...
const resourceTypeValidator = v.union(v.literal('itinerary'), v.literal('travelGuide'), v.literal('travelNote'));
/**
...
export const trackEvent = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        await ctx.db.insert('shareEventLogs', {
            ...
            resourceType: shareLink?.resourceType || args.resourceType || 'travelGuide',
...
export const getStats = query({
    ...
    handler: async (ctx, args) => {
        ...
        for (const link of shareLinks) {
            ...
            statsByPlatform[link.platform].lastShareAt = Math.max(statsByPlatform[link.platform].lastShareAt || 0, link.createdAt);
...
export const getTopShared = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.resourceType) {
            eventsQuery = eventsQuery.filter((q) => q.eq(q.field('resourceType'), args.resourceType));
        }
...
export const cleanupExpiredLinks = mutation({
    ...
    handler: async (ctx) => {
        ...
        const expiredLinks = await ctx.db
            .query('shareLinks')
            .filter((q) => q.and(q.eq(q.field('isActive'), true), q.neq(q.field('expiresAt'), undefined), q.lt(q.field('expiresAt'), now)))
            .take(100);
