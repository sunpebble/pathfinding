...
export const listRestaurants = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.minRating) {
            filtered = filtered.filter((p) => p.rating && p.rating >= args.minRating);
        }
...
export const searchRestaurants = query({
    ...
    handler: async (ctx, args) => {
        ...
        let filtered = pois.filter((p) => p.name.toLowerCase().includes(searchQuery) ||
...
export const getNearbyRestaurants = query({
    ...
    handler: async (ctx, args) => {
        ...
        const withDistance = pois
            .map((poi) => {
            const distance = haversineDistance(args.latitude, args.longitude, poi.latitude, poi.longitude);
            return { ...poi, distance };
        })
...
export const getLocalFavorites = query({
    ...
    handler: async (ctx, args) => {
        ...
        const pois = await ctx.db
            .query('pois')
            .withIndex('by_city_category', (q) => q.eq('cityId', args.cityId).eq('category', 'restaurant'))
            .collect();
...
export const getUserReview = query({
    ...
    handler: async (ctx, args) => {
        const review = await ctx.db
            .query('foodReviews')
            .withIndex('by_restaurant_user', (q) => q.eq('restaurantId', args.restaurantId).eq('userId', args.userId))
            .first();
        return review;
    },
});
...
export const addToFavorites = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query('foodFavorites')
            .withIndex('by_user_restaurant', (q) => q.eq('userId', args.userId).eq('restaurantId', args.restaurantId))
            .first();
        if (existing) {
...
export const getUserFavorites = query({
    ...
    handler: async (ctx, args) => {
        ...
        const results = await Promise.all(favorites.slice(0, limit).map(async (fav) => {
...
export const getCuisineTypes = query({
    ...
    handler: async (ctx, args) => {
        ...
        for (const poi of pois) {
            if (poi.cuisineType) {
                cuisineTypes.set(poi.cuisineType, (cuisineTypes.get(poi.cuisineType) ?? 0) + 1);
            }
        }
 ...
 */
async function updateRestaurantRating(ctx, restaurantId) {
    const reviews = await ctx.db
        .query('foodReviews')
