...
export const calculate = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const countryCodes = new Set(cities.filter((c) => c).map((c) => c.countryCode));
        const totalCountries = countryCodes.size;
        // Get all POIs visited
        let totalPois = 0;
        const transportModeCount = {};
        const poiCategoryCount = {};
        ...
        const longestTrip = sortedByDays[0]
            ...
            : undefined;
        const shortestTrip = sortedByDays.length > 0
            ? {
                ...
                startDate: sortedByDays[sortedByDays.length - 1].itinerary.startDate,
        ...
        // Get category names
        const expensesByCategory = await Promise.all(Object.values(expenseByCategory).map(async (ec) => {
        ...
        const topDestinations = await Promise.all(Object.values(cityVisitCount)
        ...
        const totalTransportModes = Object.values(transportModeCount).reduce((sum, c) => sum + c, 0);
        const preferredTransportModes = Object.entries(transportModeCount)
            .sort((a, b) => b[1] - a[1])
            .map(([mode, count]) => ({
            mode,
            count,
            percentage: totalTransportModes > 0 ? (count / totalTransportModes) * 100 : 0,
        }));
        // Calculate preferred POI categories
        const totalPoiCategories = Object.values(poiCategoryCount).reduce((sum, c) => sum + c, 0);
        const preferredPoiCategories = Object.entries(poiCategoryCount)
            .sort((a, b) => b[1] - a[1])
            .map(([category, count]) => ({
            category,
            count,
            percentage: totalPoiCategories > 0 ? (count / totalPoiCategories) * 100 : 0,
        }));
        ...
        const monthlyTripCounts = Object.entries(monthlyCount).map(([month, count]) => ({
