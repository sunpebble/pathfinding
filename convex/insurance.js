...
const insuranceTypeValidator = v.union(v.literal('comprehensive'), v.literal('medical'), v.literal('accident'), v.literal('flight_delay'), v.literal('luggage'), v.literal('cancellation'), v.literal('emergency_evacuation'));
// Risk level validator
const riskLevelValidator = v.union(v.literal('low'), v.literal('medium'), v.literal('high'), v.literal('extreme'));
// Claim type validator
const claimTypeValidator = v.union(v.literal('medical'), v.literal('accident'), v.literal('flight_delay'), v.literal('luggage_loss'), v.literal('trip_cancellation'), v.literal('emergency_evacuation'), v.literal('other'));
// ============================================
...
export const listProducts = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.type) {
            ...
            results = await ctx.db
                .query('insuranceProducts')
                .withIndex('by_domestic', (q) => q.eq('domesticOnly', args.domesticOnly))
                .filter((q) => q.eq(q.field('isActive'), true))
                .collect();
        }
        else {
...
export const getRecommendedProducts = query({
    ...
    handler: async (ctx, args) => {
        ...
        const riskProfile = await ctx.db
            ...
            .first();
        const effectiveRiskLevel = args.riskLevel || riskProfile?.riskLevel || 'medium';
        ...
        const filteredProducts = allProducts.filter((product) => {
            const coversRiskLevel = product.riskLevelCoverage.includes(effectiveRiskLevel);
            const coversDuration = args.tripDays >= product.minDays && args.tripDays <= product.maxDays;
...
export const listUserInsurance = query({
    args: {
        userId: v.string(),
        status: v.optional(v.union(v.literal('pending'), v.literal('active'), v.literal('expired'), v.literal('cancelled'), v.literal('claimed'))),
    },
    handler: async (ctx, args) => {
        let results;
        if (args.status) {
...
export const getInsuranceByItinerary = query({
    ...
    handler: async (ctx, args) => {
        ...
        const results = await Promise.all(insurances.map(async (insurance) => {
            const product = await ctx.db.get(insurance.productId);
...
export const createUserInsurance = mutation({
    args: {
        ...
        insuredPersons: v.array(v.object({
            name: v.string(),
            idType: v.union(v.literal('id_card'), v.literal('passport'), v.literal('other')),
            idNumber: v.string(),
            phone: v.optional(v.string()),
            relationship: v.union(v.literal('self'), v.literal('spouse'), v.literal('child'), v.literal('parent'), v.literal('other')),
        })),
        totalPrice: v.number(),
...
export const updateUserInsuranceStatus = mutation({
    args: {
        ...
        status: v.union(v.literal('pending'), v.literal('active'), v.literal('expired'), v.literal('cancelled'), v.literal('claimed')),
        paymentStatus: v.optional(v.union(v.literal('pending'), v.literal('paid'), v.literal('refunded'), v.literal('failed'))),
...
export const getDestinationRiskProfile = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.destinationCode) {
            return await ctx.db
                .query('destinationRiskProfiles')
                .withIndex('by_code', (q) => q.eq('destinationCode', args.destinationCode))
                .first();
        }
        return null;
    },
