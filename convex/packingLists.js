 ...
 */
// Validators
const tripTypeValidator = v.union(v.literal('leisure'), v.literal('business'), v.literal('adventure'), v.literal('beach'), v.literal('ski'), v.literal('city'), v.literal('hiking'), v.literal('other'));
const categoryValidator = v.union(v.literal('clothing'), v.literal('toiletries'), v.literal('electronics'), v.literal('documents'), v.literal('medicine'), v.literal('accessories'), v.literal('gear'), v.literal('snacks'), v.literal('other'));
const suggestedByValidator = v.union(v.literal('user'), v.literal('weather'), v.literal('activity'), v.literal('template'), v.literal('ai'));
/**
 * Permission checking helpers
 */
// Check if user can view the packing list
async function checkViewPermission(ctx, listId, userId) {
...
async function checkEditPermission(ctx, listId, userId) {
    const list = await ctx.db.get(listId);
    if (!list) {
        throw new Error('Packing list not found');
    }
    ...
    handler: async (ctx, args) => {
        ...
        const enriched = await Promise.all(data.map(async (list) => {
            ...
            return {
                ...
                packedCount,
                progress: totalItems > 0 ? Math.round((packedCount / totalItems) * 100) : 0,
            };
        }));
...
export const addItem = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existingItems = await ctx.db
            .query('packingItems')
            .withIndex('by_list_category', (q) => q.eq('packingListId', args.packingListId).eq('category', args.category))
            .collect();
    ...
    handler: async (ctx, args) => {
        ...
        for (const newItem of args.items) {
            if (categoryMaxIndex[newItem.category] === undefined) {
                const existingItems = await ctx.db
                    .query('packingItems')
                    .withIndex('by_list_category', (q) => q
                    .eq('packingListId', args.packingListId)
                    .eq('category', newItem.category))
                    .collect();
                categoryMaxIndex[newItem.category] = existingItems.reduce((max, item) => Math.max(max, item.orderIndex), -1);
            }
