 ...
 */
const visibilityValidator = v.union(v.literal('private'), v.literal('public'), v.literal('followers'));
/**
 * 权限检查辅助函数
 */
// 检查用户是否有编辑权限（只有作者可以编辑）
async function checkEditPermission(ctx, noteId, userId) {
...
// 检查游记是否对用户可见
async function _checkReadPermission(ctx, noteId, userId) {
    const note = await ctx.db.get(noteId);
    if (!note) {
        throw new Error('Travel note not found');
    }
    ...
    if (note.visibility === 'followers' && userId) {
        const following = await ctx.db
            .query('userFollows')
            .withIndex('by_follower_following', (q) => q.eq('followerId', userId).eq('followingId', note.authorId))
            .first();
        if (following) {
            return true;
        }
...
export const listPublic = query({
    args: {
        ...
        sortBy: v.optional(v.union(v.literal('latest'), v.literal('popular'), v.literal('trending'))),
    },
    handler: async (ctx, args) => {
        const page = args.page ?? 1;
        ...
        switch (sortBy) {
            ...
            case 'trending':
                // 结合时间和热度的排序
                notes.sort((a, b) => {
                    const aScore = a.likesCount * 2 +
                        a.commentsCount +
                        a.viewsCount / 10 -
                        (Date.now() - a.createdAt) / (1000 * 60 * 60 * 24);
                    const bScore = b.likesCount * 2 +
                        b.commentsCount +
                        b.viewsCount / 10 -
...
export const getById = query({
    ...
    handler: async (ctx, args) => {
        ...
        const notePois = await ctx.db
            ...
            .withIndex('by_note', (q) => q.eq('noteId', args.id))
            .collect();
        const pois = await Promise.all(notePois.map(async (np) => {
...
export const search = query({
    ...
    handler: async (ctx, args) => {
        ...
        const matchingTags = await ctx.db
            ...
            .collect();
        const tagNoteIds = new Set(matchingTags.map((t) => t.noteId));
        notes = [
            ...notes,
            ...(await Promise.all(Array.from(tagNoteIds).map((id) => ctx.db.get(id)))).filter((n) => n !== null &&
                n.visibility === 'public' &&
