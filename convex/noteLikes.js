...
export const toggle = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query('noteLikes')
            .withIndex('by_user_note', (q) => q.eq('userId', args.userId).eq('noteId', args.noteId))
            .first();
        if (existing) {
            // 取消点赞
...
export const batchCheckLikes = query({
    ...
    handler: async (ctx, args) => {
        const results = {};
        await Promise.all(args.noteIds.map(async (noteId) => {
            const like = await ctx.db
                .query('noteLikes')
                .withIndex('by_user_note', (q) => q.eq('userId', args.userId).eq('noteId', noteId))
                .first();
            results[noteId] = !!like;
        }));
        return results;
