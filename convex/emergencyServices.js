    ...
    handler: async (ctx, args) => {
        ...
        if (args.cityName) {
            const cityServices = await ctx.db
                .query('emergencyServices')
                .withIndex('by_country_city', (q) => q.eq('countryCode', args.countryCode).eq('cityName', args.cityName))
                .first();
            if (cityServices) {
                return cityServices;
            }
...
export const searchByName = query({
    ...
    handler: async (ctx, args) => {
        ...
        const matches = services.filter((s) => !s.cityName &&
    ...
    handler: async (ctx) => {
        ...
        for (const country of countries) {
            ...
            const existing = await ctx.db
                .query('emergencyServices')
                .withIndex('by_country', (q) => q.eq('countryCode', country.countryCode))
                .filter((q) => q.eq(q.field('cityName'), undefined))
                .first();
            if (!existing) {
...
export const getEmergencyGuide = query({
    args: {
        guideType: v.union(v.literal('medical'), v.literal('theft'), v.literal('lost_passport'), v.literal('natural_disaster'), v.literal('accident'), v.literal('assault'), v.literal('general')),
