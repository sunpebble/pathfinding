 ...
 */
const reportReasonValidator = v.union(v.literal('spam'), v.literal('harassment'), v.literal('inappropriate'), v.literal('misinformation'), v.literal('other'));
/**
 ...
 */
async function createNotification(ctx, params) {
...
export const listByItinerary = query({
    ...
    handler: async (ctx, args) => {
        ...
        const enriched = await Promise.all(paginatedComments.map(async (comment) => {
            ...
            if (args.userId) {
                const like = await ctx.db
                    .query('commentLikes')
                    .withIndex('by_comment_user', (q) => q.eq('commentId', comment._id).eq('userId', args.userId))
                    .first();
                isLikedByUser = !!like;
            }
            return {
    ...
    handler: async (ctx, args) => {
        ...
        const enriched = await Promise.all(replies.map(async (reply) => {
            ...
            if (args.userId) {
                const like = await ctx.db
                    .query('commentLikes')
                    .withIndex('by_comment_user', (q) => q.eq('commentId', reply._id).eq('userId', args.userId))
                    .first();
                isLikedByUser = !!like;
            }
            return {
...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.content.length > 2000) {
            throw new Error('Comment content exceeds maximum length of 2000 characters');
        }
