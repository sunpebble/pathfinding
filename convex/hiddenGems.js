...
const popularityLevelValidator = v.union(v.literal('hidden'), v.literal('emerging'), v.literal('moderate'), v.literal('popular'), v.literal('crowded'));
const userSubmittedPoiStatusValidator = v.union(v.literal('pending'), v.literal('approved'), v.literal('rejected'), v.literal('merged'));
// ============================================
...
export const listHiddenGems = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.cityId) {
            pois = await ctx.db
                .query('pois')
                .withIndex('by_city_hidden_gem', (q) => q.eq('cityId', args.cityId).eq('isHiddenGem', true))
                .collect();
        }
        else {
            pois = await ctx.db
        ...
        if (args.minHiddenGemRating !== undefined) {
            pois = pois.filter((poi) => poi.hiddenGemRating !== undefined &&
                poi.hiddenGemRating >= args.minHiddenGemRating);
        }
        if (args.onlyLocalRecommended) {
            pois = pois.filter((poi) => poi.localRecommendation?.isLocalRecommended === true);
        }
...
export const getByPopularityLevel = query({
    ...
    handler: async (ctx, args) => {
        ...
        let pois = await ctx.db
            .query('pois')
            .withIndex('by_popularity_level', (q) => q.eq('popularityLevel', args.popularityLevel))
            .take(limit * 2);
        if (args.cityId) {
...
export const voteOnUserSubmittedPoi = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existingVote = await ctx.db
            ...
            .withIndex('by_poi_user', (q) => q.eq('poiId', args.poiId).eq('userId', args.userId))
            .first();
...
export const rateHiddenGem = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (existingRating) {
            ...
            const oldTotal = (poi.hiddenGemRating ?? 0) * (poi.hiddenGemRatingCount ?? 0);
