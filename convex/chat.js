 ...
 */
const roleValidator = v.union(v.literal('user'), v.literal('assistant'), v.literal('system'));
...
export const listSessions = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.includeArchived) {
            ...
            sessions = await ctx.db
                .query('chatSessions')
                .withIndex('by_user_archived', (q) => q.eq('userId', args.userId).eq('isArchived', false))
                .order('desc')
                .take(limit);
        }
        return sessions;
    ...
    handler: async (ctx, args) => {
        ...
        if (session) {
            ...
            if (session.title === '新对话' &&
                ...
                const newTitle = args.content.slice(0, 30) + (args.content.length > 30 ? '...' : '');
        ...
        if (session?.itineraryId) {
            ...
            if (itinerary) {
                ...
                const days = await ctx.db
                    .query('itineraryDays')
                    .withIndex('by_itinerary', (q) => q.eq('itineraryId', session.itineraryId))
                    .collect();
                itineraryContext = {
            ...
            if (it) {
                ...
                const daysWithItems = await Promise.all(days.map(async (day) => {
                    const items = await ctx.db
                        ...
                        .collect();
                    const itemsWithPoi = await Promise.all(items.map(async (item) => {
