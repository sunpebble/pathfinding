...
const travelStyleValidator = v.union(v.literal('adventure'), v.literal('relaxation'), v.literal('culture'), v.literal('food'), v.literal('nature'), v.literal('shopping'), v.literal('photography'), v.literal('budget'), v.literal('luxury'));
const ageRangeValidator = v.union(v.literal('18-25'), v.literal('26-35'), v.literal('36-45'), v.literal('46-55'), v.literal('55+'));
const genderValidator = v.union(v.literal('male'), v.literal('female'), v.literal('other'), v.literal('any'));
const requestStatusValidator = v.union(v.literal('active'), v.literal('paused'), v.literal('fulfilled'), v.literal('cancelled'), v.literal('expired'));
const applicationStatusValidator = v.union(v.literal('pending'), v.literal('accepted'), v.literal('rejected'), v.literal('withdrawn'), v.literal('expired'));
const budgetRangeValidator = v.union(v.literal('budget'), v.literal('moderate'), v.literal('comfortable'), v.literal('luxury'));
// ============================================
...
export const listRequests = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.destination) {
            ...
            requests = requests.filter((r) => r.destination.toLowerCase().includes(destLower));
        }
        if (args.cityId) {
        ...
        if (args.travelStyles && args.travelStyles.length > 0) {
            requests = requests.filter((r) => {
                if (!r.travelStyles)
                    return false;
                return args.travelStyles.some((style) => r.travelStyles.includes(style));
            });
        }
        const total = requests.length;
        const data = requests.slice(offset, offset + pageSize);
        // Enrich with user info and city data
        const enriched = await Promise.all(data.map(async (request) => {
...
export const applyToRequest = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            ...
            .collect();
        const alreadyApplied = existing.find((a) => a.applicantId === args.applicantId && a.status !== 'withdrawn');
        if (alreadyApplied) {
            throw new Error('You have already applied to this request');
        }
        // Calculate match score
        const matchScore = await calculateMatchScore(ctx, args.applicantId, request);
        const now = Date.now();
 ...
 */
async function calculateMatchScore(ctx, applicantId, request) {
    ...
    if (request.travelStyles && applicantPrefs?.travelStyles) {
        const matchingStyles = request.travelStyles.filter((style) => applicantPrefs.travelStyles.includes(style));
        factors.styleMatch = Math.round((matchingStyles.length /
            Math.max(request.travelStyles.length, applicantPrefs.travelStyles.length)) *
            100);
    ...
    if (request.preferredAgeRange && applicantPrefs?.ageRange) {
        factors.ageMatch = request.preferredAgeRange.includes(applicantPrefs.ageRange)
            ? 100
            : 50;
    ...
    if (request.budgetRange && applicantPrefs?.accommodationPreference) {
        ...
        factors.budgetMatch = compatibleAccommodations.includes(applicantPrefs.accommodationPreference)
            ? 100
            : 60;
...
export const listMatches = query({
    ...
    handler: async (ctx, args) => {
        ...
        let allMatches = [
            ...ownerMatches.map((m) => ({ ...m, userRole: 'owner' })),
            ...partnerMatches.map((m) => ({ ...m, userRole: 'partner' })),
        ];
        if (args.status) {
        ...
        const enriched = await Promise.all(data.map(async (match) => {
            const otherUserId = match.userRole === 'owner' ? match.partnerId : match.requestOwnerId;
            const profile = await ctx.db
                .query('profiles')
 ...
 */
async function updateTrustScore(ctx, userId) {
    ...
    // Calculate overall score
    const overallScore = verificationScore + feedbackScore + activityScore + responseScore;
    // Determine badges
    const badges = [];
    if (verifiedCount > 0)
...
export const listSavedRequests = query({
    ...
    handler: async (ctx, args) => {
        ...
        const enriched = await Promise.all(data.map(async (save) => {
