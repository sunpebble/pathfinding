...
const statusValidator = v.union(v.literal('pending'), v.literal('generating'), v.literal('completed'), v.literal('failed'));
...
export const list = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.name) {
            ...
            datasets = datasets.filter((d) => d.name.toLowerCase().includes(nameLower));
        }
        if (args.status) {
...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query('trainingDatasets')
            .filter((q) => q.and(q.eq(q.field('name'), args.name), q.eq(q.field('version'), args.version)))
            .first();
        if (existing) {
            throw new Error(`Dataset ${args.name} version ${args.version} already exists`);
        }
