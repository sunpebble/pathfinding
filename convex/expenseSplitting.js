 ...
 */
// Validators
const expenseCategoryValidator = v.union(v.literal('food'), v.literal('transport'), v.literal('accommodation'), v.literal('tickets'), v.literal('shopping'), v.literal('other'));
const splitTypeValidator = v.union(v.literal('equal'), v.literal('exact'), v.literal('percentage'), v.literal('shares'));
// ============================================
// Permission Helpers
// ============================================
async function checkItineraryAccess(ctx, itineraryId, userId) {
...
async function checkMemberAccess(ctx, memberId, userId) {
...
export const addMember = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.memberUserId) {
            const existingByUserId = existing.find((m) => m.userId === args.memberUserId);
            if (existingByUserId) {
...
export const addOwnerAsMember = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query('tripMembers')
            .withIndex('by_itinerary_user', (q) => q.eq('itineraryId', args.itineraryId).eq('userId', args.userId))
            .first();
        if (existing) {
            return existing._id;
        }
...
export const listExpenses = query({
    ...
    handler: async (ctx, args) => {
        ...
        const enriched = await Promise.all(expenses.map(async (expense) => {
            ...
            const participantsWithInfo = await Promise.all(participants.map(async (p) => {
...
export const addExpense = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const participantsWithAmounts = calculateSplitAmounts(args.amount, args.splitType, args.participants);
        const now = Date.now();
        // Create the expense
...
export const updateExpense = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.participants !== undefined) {
            ...
            const participantsWithAmounts = calculateSplitAmounts(amount, splitType, args.participants);
...
export const getExpenseSummary = query({
    ...
    handler: async (ctx, args) => {
        ...
        const breakdown = categories.map((category) => ({
            ...
            percentage: total > 0
                ? Math.round(((categoryTotals.get(category) ?? 0) / total) * 100)
                : 0,
        }));
        return {
            total,
...
export const listSettlements = query({
    ...
    handler: async (ctx, args) => {
        ...
        const enriched = await Promise.all(settlements.map(async (settlement) => {
 ...
 */
function calculateSplitAmounts(totalAmount, splitType, participants) {
    const result = [];
    switch (splitType) {
        case 'equal': {
 ...
 */
function calculateMinimumTransfers(balances, memberInfo) {
    const suggestions = [];
