...
const notificationTypeValidator = v.union(v.literal('comment'), v.literal('reply'), v.literal('like'), v.literal('mention'), v.literal('new_follower'), v.literal('following_itinerary'), v.literal('itinerary_reminder'), v.literal('flight_status'), v.literal('weather_alert'), v.literal('social_interaction'));
const referenceTypeValidator = v.union(v.literal('itinerary'), v.literal('comment'), v.literal('user'), v.literal('flight'), v.literal('weather'));
...
export const updateSettings = mutation({
    args: {
        ...
        itineraryReminders: v.optional(v.object({
            enabled: v.boolean(),
            advanceHours: v.number(),
        })),
        flightAlerts: v.optional(v.object({
            enabled: v.boolean(),
            statusChanges: v.boolean(),
            checkInReminders: v.boolean(),
            boardingReminders: v.boolean(),
        })),
        weatherAlerts: v.optional(v.object({
            enabled: v.boolean(),
            severeOnly: v.boolean(),
        })),
        socialNotifications: v.optional(v.object({
            enabled: v.boolean(),
            comments: v.boolean(),
            likes: v.boolean(),
            follows: v.boolean(),
...
export const getPendingScheduled = query({
    ...
    handler: async (ctx, args) => {
        ...
        const scheduled = await ctx.db
            ...
            .withIndex('by_status_scheduled', (q) => q.eq('status', 'pending').lte('scheduledFor', args.beforeTime))
            .take(limit);
        return scheduled;
    },
});
...
export const cancelScheduledByReference = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        for (const notification of scheduled) {
            if (notification.referenceType === args.referenceType &&
...
export const sendPendingReminders = internalMutation({
    handler: async (ctx) => {
        ...
        const pendingNotifications = await ctx.db
            .query('scheduledNotifications')
            .filter((q) => q.and(q.eq(q.field('status'), 'pending'), q.lte(q.field('scheduledFor'), now)))
            .collect();
        let sentCount = 0;
...
export const cleanupOldNotifications = internalMutation({
    handler: async (ctx) => {
        ...
        const oldNotifications = await ctx.db
            .query('notifications')
            .filter((q) => q.and(q.eq(q.field('isRead'), true), q.lt(q.field('createdAt'), thirtyDaysAgo)))
            .collect();
        // Delete them
        let deletedCount = 0;
