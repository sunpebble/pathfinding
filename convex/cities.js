...
export const updateEncyclopedia = mutation({
    args: {
        ...
        customs: v.optional(v.array(v.object({
            category: v.union(v.literal('etiquette'), v.literal('religion'), v.literal('dining'), v.literal('dress'), v.literal('gift'), v.literal('gesture'), v.literal('general')),
            title: v.string(),
            titleEn: v.optional(v.string()),
            description: v.string(),
    ...
    handler: async (ctx, args) => {
        const { id, ...updates } = args;
        // Filter out undefined values
        const filteredUpdates = Object.fromEntries(Object.entries(updates).filter(([, val]) => val !== undefined));
...
export const listWithEncyclopedia = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.countryCode) {
            citiesQuery = citiesQuery.withIndex('by_country', (q) => q.eq('countryCode', args.countryCode));
        }
        const cities = await citiesQuery.take(limit);
        // Fetch encyclopedia data for each city
        const citiesWithEncyclopedia = await Promise.all(cities.map(async (city) => {
            const encyclopedia = await ctx.db
                .query('cityEncyclopedia')
