...
export const updateDisplayFormat = mutation({
    ...
    handler: async (ctx, args) => {
        const existing = await ctx.db
            ...
            .first();
        if (!existing) {
            throw new Error('User timezone settings not found. Please set home timezone first.');
        }
        const updates = {
...
export const addSavedClock = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const alreadySaved = existing.savedClocks.some((clock) => clock.cityId === args.cityId);
        if (alreadySaved) {
            throw new Error('City is already in your world clock.');
        }
        // Add to the end with next sort order
        const maxOrder = existing.savedClocks.reduce((max, clock) => Math.max(max, clock.sortOrder), 0);
        const newClock = {
            cityId: args.cityId,
            label: args.label,
...
export const removeSavedClock = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const filteredClocks = existing.savedClocks.filter((clock) => clock.cityId !== args.cityId);
        await ctx.db.patch(existing._id, {
...
export const updateSavedClockLabel = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const updatedClocks = existing.savedClocks.map((clock) => clock.cityId === args.cityId ? { ...clock, label: args.label } : clock);
        await ctx.db.patch(existing._id, {
...
export const reorderSavedClocks = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const clockMap = new Map(existing.savedClocks.map((clock) => [clock.cityId, clock]));
...
export const getWorldClock = query({
    ...
    handler: async (ctx, args) => {
        ...
        const clockCities = await Promise.all(settings.savedClocks.map(async (clock) => {
