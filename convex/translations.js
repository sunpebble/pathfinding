...
export const listPhrasesByCategory = query({
    args: {
        category: v.union(v.literal('greeting'), v.literal('transportation'), v.literal('dining'), v.literal('shopping'), v.literal('accommodation'), v.literal('emergency'), v.literal('directions'), v.literal('numbers'), v.literal('time'), v.literal('common')),
...
export const searchPhrases = query({
    args: {
        query: v.string(),
        category: v.optional(v.union(v.literal('greeting'), v.literal('transportation'), v.literal('dining'), v.literal('shopping'), v.literal('accommodation'), v.literal('emergency'), v.literal('directions'), v.literal('numbers'), v.literal('time'), v.literal('common'))),
        sourceLang: v.optional(v.string()),
        limit: v.optional(v.number()),
    },
    handler: async (ctx, args) => {
...
export const listSavedTranslations = query({
    args: {
        userId: v.string(),
        translationType: v.optional(v.union(v.literal('text'), v.literal('photo'), v.literal('voice'))),
    ...
    handler: async (ctx, args) => {
        ...
        if (args.favoritesOnly) {
            q = ctx.db
                ...
                .withIndex('by_user_favorite', (q) => q.eq('userId', args.userId).eq('isFavorite', true));
        }
...
export const searchSavedTranslations = query({
    ...
    handler: async (ctx, args) => {
        const results = await ctx.db
            .query('savedTranslations')
            .withSearchIndex('search_saved', (q) => q.search('sourceText', args.query).eq('userId', args.userId))
            .take(args.limit || 20);
        return results;
    },
});
...
export const saveTranslation = mutation({
    args: {
        ...
        targetText: v.string(),
        targetLang: v.string(),
        translationType: v.union(v.literal('text'), v.literal('photo'), v.literal('voice')),
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            ...
            .filter((q) => q.and(q.eq(q.field('sourceText'), args.sourceText), q.eq(q.field('sourceLang'), args.sourceLang), q.eq(q.field('targetLang'), args.targetLang)))
            .first();
        if (existing) {
...
export const getUserPacks = query({
    ...
    handler: async (ctx, args) => {
        const userPacks = await ctx.db
            ...
            .collect();
        // Fetch pack details
        const result = await Promise.all(userPacks.map(async (up) => {
...
export const recordPackDownload = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query('userOfflinePacks')
            .withIndex('by_user_pack', (q) => q.eq('userId', args.userId).eq('packId', args.packId))
            .first();
        if (existing) {
            // Update version
            await ctx.db.patch(existing._id, {
...
export const batchCreatePhrases = mutation({
    ...
    handler: async (ctx, args) => {
        const ids = await Promise.all(args.phrases.map((phrase) => ctx.db.insert('translationPhrases', phrase)));
        return ids;
    },
});
/**
...
export const supportedLanguages = ['zh', 'en', 'ja'];
const contentLanguageValidator = v.union(v.literal('zh'), v.literal('en'), v.literal('ja'));
const entityTypeValidator = v.union(v.literal('poi'), v.literal('city'), v.literal('travelGuide'), v.literal('itinerary'));
/**
...
export const getContentTranslation = query({
    ...
    handler: async (ctx, args) => {
        return await ctx.db
            .query('contentTranslations')
            .withIndex('by_entity_field_language', (q) => q
        ...
        for (const translation of args.translations) {
            ...
            if (existing) {
                await ctx.db.patch(existing._id, {
                    ...
                    isAutoTranslated: translation.isAutoTranslated ?? existing.isAutoTranslated,
                    translatedBy: args.translatedBy,
                    updatedAt: now,
                });
