...
const reportReasonValidator = v.union(v.literal('spam'), v.literal('inappropriate'), v.literal('misleading'), v.literal('off_topic'), v.literal('harassment'), v.literal('other'));
const questionStatusValidator = v.union(v.literal('open'), v.literal('closed'), v.literal('resolved'));
/**
 ...
 */
async function checkPoiExists(ctx, poiId) {
 ...
 */
async function createQANotification(ctx, params) {
    ...
    await ctx.db.insert('notifications', {
        ...
        type: params.type, // Using 'comment' type for compatibility
...
export const listQuestionsByPoi = query({
    args: {
        ...
        sortBy: v.optional(v.union(v.literal('newest'), v.literal('oldest'), v.literal('most_upvoted'), v.literal('most_active'))),
    ...
    handler: async (ctx, args) => {
        ...
        const enriched = await Promise.all(paginatedQuestions.map(async (question) => {
            ...
            if (args.userId) {
                const vote = await ctx.db
                    ...
                    .withIndex('by_question_user', (q) => q.eq('questionId', question._id).eq('userId', args.userId))
                    .first();
...
export const getQuestionById = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.userId) {
            const vote = await ctx.db
                .query('poiQuestionVotes')
                .withIndex('by_question_user', (q) => q.eq('questionId', args.id).eq('userId', args.userId))
                .first();
            userVote = vote?.voteType ?? null;
        }
        // Get POI info
...
export const searchQuestions = query({
    ...
    handler: async (ctx, args) => {
        ...
        const searchResults = await ctx.db
            ...
            .take(limit);
        // Enrich results
        const enriched = await Promise.all(searchResults.map(async (question) => {
            const profile = await getUserProfile(ctx, question.userId);
            const poi = await ctx.db.get(question.poiId);
            return {
                ...question,
...
export const getMyQuestions = query({
    ...
    handler: async (ctx, args) => {
        ...
        const enriched = await Promise.all(paginated.map(async (question) => {
            const poi = await ctx.db.get(question.poiId);
            return {
                ...question,
                id: question._id,
...
export const listAnswersByQuestion = query({
    args: {
        ...
        page: v.optional(v.number()),
        pageSize: v.optional(v.number()),
        sortBy: v.optional(v.union(v.literal('newest'), v.literal('oldest'), v.literal('most_upvoted'))),
        userId: v.optional(v.string()),
    },
    handler: async (ctx, args) => {
        const page = args.page ?? 1;
        ...
        // Enrich with user info and vote status
        const enriched = await Promise.all(paginatedAnswers.map(async (answer) => {
            const profile = await getUserProfile(ctx, answer.userId);
            let userVote = null;
            if (args.userId) {
                const vote = await ctx.db
                    .query('poiAnswerVotes')
                    .withIndex('by_answer_user', (q) => q.eq('answerId', answer._id).eq('userId', args.userId))
                    .first();
                userVote = vote?.voteType ?? null;
            }
            return {
...
export const getAnswerById = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.userId) {
            const vote = await ctx.db
                .query('poiAnswerVotes')
                .withIndex('by_answer_user', (q) => q.eq('answerId', args.id).eq('userId', args.userId))
                .first();
            userVote = vote?.voteType ?? null;
        }
        return {
...
export const getMyAnswers = query({
    ...
    handler: async (ctx, args) => {
        ...
        const enriched = await Promise.all(paginated.map(async (answer) => {
...
export const createQuestion = mutation({
    args: {
        ...
        title: v.string(),
        content: v.string(),
        category: v.optional(v.union(v.literal('general'), v.literal('transportation'), v.literal('timing'), v.literal('pricing'), v.literal('tips'), v.literal('food'), v.literal('accommodation'), v.literal('safety'), v.literal('other'))),
...
export const voteQuestion = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existingVote = await ctx.db
            .query('poiQuestionVotes')
            .withIndex('by_question_user', (q) => q.eq('questionId', args.questionId).eq('userId', args.userId))
            .first();
        if (existingVote) {
            if (existingVote.voteType === args.voteType) {
                ...
                // Update counts
                const countUpdate = args.voteType === 'up'
...
export const voteAnswer = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existingVote = await ctx.db
            .query('poiAnswerVotes')
            .withIndex('by_answer_user', (q) => q.eq('answerId', args.answerId).eq('userId', args.userId))
            .first();
        if (existingVote) {
            if (existingVote.voteType === args.voteType) {
                // Remove vote (toggle off)
...
export const reportQA = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.targetType === 'question') {
            const question = await ctx.db.get(args.targetId);
            if (!question) {
                throw new Error('Question not found');
            }
            ...
            const answer = await ctx.db.get(args.targetId);
            if (!answer) {
                throw new Error('Answer not found');
            }
            await ctx.db.patch(args.targetId, {
