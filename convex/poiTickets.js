 ...
 */
const ticketTypeValidator = v.union(v.literal('adult'), v.literal('student'), v.literal('senior'), v.literal('child'), v.literal('group'), v.literal('family'), v.literal('vip'), v.literal('free'), v.literal('other'));
const stockStatusValidator = v.union(v.literal('in_stock'), v.literal('low_stock'), v.literal('sold_out'), v.literal('unknown'));
// ============================================
// POI Tickets Queries
// ============================================
/**
    ...
    handler: async (ctx, args) => {
        ...
        if (args.activeOnly) {
            tickets = await ctx.db
                .query('poiTickets')
                .withIndex('by_poi_active', (q) => q.eq('poiId', args.poiId).eq('isActive', true))
                .collect();
        }
        else {
            tickets = await ctx.db
...
export const listByType = query({
    ...
    handler: async (ctx, args) => {
        return await ctx.db
            .query('poiTickets')
            .withIndex('by_poi_type', (q) => q.eq('poiId', args.poiId).eq('ticketType', args.ticketType))
            .collect();
    },
});
/**
...
export const getPriceRange = query({
    ...
    handler: async (ctx, args) => {
        ...
        const hasFreeTickets = tickets.some((t) => t.ticketType === 'free' || t.price === 0);
        return {
            minPrice,
            maxPrice,
            currency,
...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existingTickets = await ctx.db
            ...
            .withIndex('by_poi', (q) => q.eq('poiId', args.poiId))
            .collect();
        const maxSortOrder = existingTickets.length > 0
