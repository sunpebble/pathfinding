...
const activityTypeValidator = v.union(v.literal('new_itinerary'), v.literal('update_itinerary'), v.literal('like_itinerary'), v.literal('comment_itinerary'), v.literal('copy_itinerary'), v.literal('follow_user'));
const visibilityValidator = v.union(v.literal('public'), v.literal('followers'));
// ============================================
// Queries
// ============================================
/**
...
export const getPublicFeed = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.activityType) {
            activitiesQuery = activitiesQuery.filter((q) => q.eq(q.field('activityType'), args.activityType));
        }
...
export const getTrendingFeed = query({
    ...
    handler: async (ctx, args) => {
        ...
        const activities = await ctx.db
            ...
            .withIndex('by_visibility_created', (q) => q.eq('visibility', 'public'))
            .order('desc')
            .filter((q) => q.and(q.gt(q.field('createdAt'), cutoffTime), q.eq(q.field('activityType'), 'new_itinerary')))
            .take(limit * 3);
        // Calculate engagement score and sort
        const scoredActivities = activities.map((activity) => ({
            ...activity,
            engagementScore: (activity.likesCount || 0) * 2 + (activity.commentsCount || 0) * 3,
        }));
    ...
    handler: async (ctx, args) => {
        ...
        if (activity) {
            await ctx.db.patch(activity._id, {
                ...
                commentsCount: (activity.commentsCount || 0) + (args.commentsIncrement || 0),
                updatedAt: Date.now(),
            });
        }
    },
...
export const getFollowers = query({
    ...
    handler: async (ctx, args) => {
        ...
        const enrichedFollows = await Promise.all(data.map(async (follow) => {
