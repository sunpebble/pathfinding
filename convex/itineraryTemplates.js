 ...
 */
const visibilityValidator = v.union(v.literal('private'), v.literal('public'), v.literal('unlisted'));
const templateTypeValidator = v.union(v.literal('preset'), v.literal('user'));
const poiTypeValidator = v.union(v.literal('attraction'), v.literal('restaurant'), v.literal('hotel'), v.literal('transportation'), v.literal('activity'), v.literal('shopping'));
...
export const listPublicTemplates = query({
    args: {
        ...
        sortBy: v.optional(v.union(v.literal('popular'), v.literal('newest'), v.literal('most_used'))),
    },
    handler: async (ctx, args) => {
        ...
        const enriched = await Promise.all(data.map(async (template) => {
    ...
    handler: async (ctx, args) => {
        ...
        if (args.userId) {
            const like = await ctx.db
                .query('templateLikes')
                .withIndex('by_template_user', (q) => q.eq('templateId', args.id).eq('userId', args.userId))
                .first();
            isLiked = !!like;
...
export const listSavedTemplates = query({
    ...
    handler: async (ctx, args) => {
        ...
        const templates = await Promise.all(paginatedSaves.map(async (save) => {
...
export const searchTemplates = query({
    ...
    handler: async (ctx, args) => {
        ...
        const filtered = templates.filter((t) => {
            ...
            const tagMatch = t.tags?.some((tag) => tag.toLowerCase().includes(searchQuery));
            const destMatch = t.destinations?.some((dest) => dest.toLowerCase().includes(searchQuery));
...
export const getRecommendedTemplates = query({
    ...
    handler: async (ctx, args) => {
        ...
        // Enrich with category info
        const enriched = await Promise.all(selectedTemplates.map(async (template) => {
            const category = await ctx.db.get(template.categoryId);
            return {
                ...template,
                categoryName: category?.name,
...
export const toggleLike = mutation({
    ...
    handler: async (ctx, args) => {
        const existing = await ctx.db
            .query('templateLikes')
            .withIndex('by_template_user', (q) => q.eq('templateId', args.templateId).eq('userId', args.userId))
            .first();
...
export const saveItineraryAsTemplate = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const _templateDays = await Promise.all(days.map(async (day) => {
            ...
            const pois = await Promise.all(items.map(async (item) => {
                const poi = await ctx.db.get(item.poiId);
                return {
                    name: poi?.name ?? 'Unknown',
                    type: poi?.category ?? 'attraction',
                    description: item.notes,
                    suggestedDuration: item.startTime && item.endTime
