...
export const listByCityAndType = query({
    args: {
        cityId: v.id('cities'),
        facilityType: v.union(v.literal('hospital'), v.literal('clinic'), v.literal('pharmacy'), v.literal('emergency'), v.literal('dental'), v.literal('specialist')),
    },
    handler: async (ctx, args) => {
        return await ctx.db
            .query('medicalFacilities')
            .withIndex('by_city_type', (q) => q.eq('cityId', args.cityId).eq('facilityType', args.facilityType))
            .filter((q) => q.eq(q.field('isActive'), true))
            .collect();
    },
});
...
export const searchNearby = query({
    args: {
        ...
        facilityType: v.optional(v.union(v.literal('hospital'), v.literal('clinic'), v.literal('pharmacy'), v.literal('emergency'), v.literal('dental'), v.literal('specialist'))),
    },
    handler: async (ctx, args) => {
        ...
        if (args.facilityType) {
            facilities = facilities.filter((f) => f.facilityType === args.facilityType);
        }
        // Calculate distance and filter
        const filteredFacilities = facilities
            .map((facility) => {
            const distance = calculateDistance(args.latitude, args.longitude, facility.latitude, facility.longitude);
