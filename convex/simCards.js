 ...
 */
// Card type validator
const cardTypeValidator = v.union(v.literal('physical'), // Physical SIM card
v.literal('esim'), // eSIM
v.literal('wifi_device') // Portable WiFi device
);
// Coverage type validator
const coverageTypeValidator = v.union(v.literal('single_country'), // Single country
...
export const list = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.destination) {
            results = results.filter((card) => card.destinations.includes(args.destination) ||
                card.destinationNames?.some((name) => name.toLowerCase().includes(args.destination.toLowerCase())));
        }
        // Filter by card type
        if (args.cardType) {
        ...
        if (args.coverageType) {
            results = results.filter((card) => card.coverageType === args.coverageType);
        }
        // Filter by minimum data
        if (args.minData) {
            results = results.filter((card) => card.dataPlans.some((plan) => plan.isUnlimited ||
                (plan.dataAmountBytes && plan.dataAmountBytes >= args.minData)));
        }
        // Filter by max price
        if (args.maxPrice) {
            results = results.filter((card) => card.dataPlans.some((plan) => plan.price <= args.maxPrice));
        }
        // Filter by minimum validity days
        if (args.minDays) {
            results = results.filter((card) => card.dataPlans.some((plan) => plan.validityDays >= args.minDays));
        }
        // Filter by voice support
        if (args.includesVoice !== undefined) {
            results = results.filter((card) => card.includesVoice === args.includesVoice);
        }
...
export const getRecommended = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.tripDurationDays) {
            results = results.filter((card) => card.dataPlans.some((plan) => plan.validityDays >= args.tripDurationDays));
        }
        // Sort by preference
        results.sort((a, b) => {
            let scoreA = 0;
...
export const search = query({
    ...
    handler: async (ctx, args) => {
        ...
        results = results.filter((card) => card.name.toLowerCase().includes(searchLower) ||
...
export const getByRegion = query({
    ...
    handler: async (ctx, args) => {
        ...
        results = results.filter((card) => card.isActive);
        // Filter by region
        results = results.filter((card) => card.coverageType === 'regional' &&
...
export const update = mutation({
    args: {
        ...
        coverageType: v.optional(coverageTypeValidator),
        regionName: v.optional(v.string()),
        dataPlans: v.optional(v.array(v.object({
            dataAmount: v.string(),
            dataAmountBytes: v.optional(v.number()),
            isUnlimited: v.boolean(),
            throttledSpeedAfterLimit: v.optional(v.string()),
