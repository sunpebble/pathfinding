...
export const getAvailableYears = query({
    ...
    handler: async (ctx, args) => {
        const itineraries = await ctx.db
            ...
            .withIndex('by_user', (q) => q.eq('userId', args.userId))
            .collect();
        const years = new Set(itineraries.map((it) => new Date(it.startDate).getFullYear()));
...
export const generate = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        try {
            ...
            const expenseBreakdown = await Promise.all(Object.values(expenseByCategory).map(async (ec) => {
            ...
            for (const it of itineraries) {
                const tripExpenses = yearExpenses.filter((e) => e.itineraryId === it._id);
            ...
            const sortedByDate = [...itineraries].sort((a, b) => new Date(a.startDate).getTime() - new Date(b.startDate).getTime());
            ...
            const topCities = await Promise.all(Object.values(cityVisitCount)
            ...
            const previousReview = await ctx.db
                .query('yearlyReviews')
                .withIndex('by_user_year', (q) => q.eq('userId', args.userId).eq('year', previousYear))
                .first();
            if (previousReview && previousReview.status === 'ready') {
                ...
                yearOverYear = {
                    ...
                    expensesChange: calcChange(totalExpenses, previousReview.totalExpenses),
