...
export default defineSchema({
    ...
    activityFeed: defineTable({
        ...
        // Activity type
        activityType: v.union(v.literal('new_itinerary'), // Created a new itinerary
        ...
        // Visibility
        visibility: v.union(v.literal('public'), // Anyone can see
    ...
    cityEncyclopedia: defineTable({
        ...
        // Best Travel Time
        bestTravelTime: v.optional(v.object({
            seasons: v.array(v.union(v.literal('spring'), v.literal('summer'), v.literal('autumn'), v.literal('winter'), v.literal('all_year'))),
            months: v.array(v.number()), // 1-12
            description: v.string(),
            descriptionEn: v.optional(v.string()),
            weatherNotes: v.optional(v.string()),
            crowdLevel: v.optional(v.union(v.literal('low'), v.literal('medium'), v.literal('high'))),
            priceLevel: v.optional(v.union(v.literal('low'), v.literal('medium'), v.literal('high'))),
        })),
        // Local Customs and Taboos
        customs: v.array(v.object({
            category: v.union(v.literal('etiquette'), v.literal('religion'), v.literal('dining'), v.literal('dress'), v.literal('gift'), v.literal('gesture'), v.literal('general')),
            title: v.string(),
        ...
        practicalInfo: v.optional(v.object({
            voltage: v.string(),
            ...
            waterSafety: v.union(v.literal('safe'), v.literal('boil'), v.literal('bottled')),
    ...
    pois: defineTable({
        ...
        name: v.string(),
        nameEn: v.optional(v.string()),
        category: v.union(v.literal('attraction'), v.literal('restaurant'), v.literal('hotel'), v.literal('shopping'), v.literal('other')),
        cityId: v.id('cities'),
        ...
        bestVisitTime: v.optional(v.object({
        ...
        localRecommendation: v.optional(v.object({
        ...
        popularityLevel: v.optional(v.union(v.literal('hidden'), // Very few visitors know about it
    ...
    userSubmittedPois: defineTable({
        ...
        // Moderation
        status: v.union(v.literal('pending'), // Awaiting review
    ...
    poiBusinessHoursReminders: defineTable({
        ...
        reminderType: v.union(v.literal('opening'), // Remind when POI opens
    ...
    collaboratorPresence: defineTable({
        ...
        selectedElements: v.optional(v.array(v.object({
    ...
    editOperations: defineTable({
        itineraryId: v.id('itineraries'),
        userId: v.string(),
        operationType: v.union(v.literal('create'), v.literal('update'), v.literal('delete'), v.literal('reorder')),
        targetType: v.union(v.literal('itinerary'), v.literal('day'), v.literal('item')),
        targetId: v.string(),
        changes: v.any(), // The actual changes made
        timestamp: v.number(),
        version: v.number(), // For optimistic concurrency control
        status: v.union(v.literal('pending'), v.literal('applied'), v.literal('conflicted'), v.literal('rejected')),
        conflictResolution: v.optional(v.object({
            resolvedBy: v.string(), // userId
            resolvedAt: v.number(),
            resolution: v.union(v.literal('accept_mine'), v.literal('accept_theirs'), v.literal('merge')),
        })),
    })
        .index('by_itinerary', ['itineraryId'])
    ...
    itineraryCopyHistory: defineTable({
        ...
        copyType: v.union(v.literal('full'), // Full copy of all days
    ...
    itineraryItems: defineTable({
        ...
        transportMode: v.union(v.literal('walking'), v.literal('driving'), v.literal('transit'), v.literal('cycling'), v.literal('taxi')),
    ...
    poiTickets: defineTable({
        ...
        ticketType: v.union(v.literal('adult'), // 成人票
        ...
        stockStatus: v.optional(v.union(v.literal('in_stock'), // 有票
    ...
    ticketReminders: defineTable({
        ...
        // 提醒信息
        reminderType: v.union(v.literal('reservation_open'), // 开放预约提醒
    ...
    travelGuides: defineTable({
        sourcePlatform: v.union(v.literal('xiaohongshu'), v.literal('weibo'), v.literal('ctrip'), v.literal('douyin'), v.literal('tripadvisor'), v.literal('qunar'), v.literal('tongcheng'), v.literal('mafengwo')),
        sourceExternalId: v.string(),
        ...
        enrichmentStatus: v.optional(v.union(v.literal('pending'), v.literal('processing'), v.literal('completed'), v.literal('failed'))),
        ...
        geocodingMetrics: v.optional(v.object({
            ...
            sourceDistribution: v.optional(v.object({
    ...
    commentReports: defineTable({
        commentId: v.id('itineraryComments'),
        userId: v.string(), // Auth user ID
        reason: v.union(v.literal('spam'), v.literal('harassment'), v.literal('inappropriate'), v.literal('misinformation'), v.literal('other')),
        description: v.optional(v.string()),
    ...
    notifications: defineTable({
        ...
        type: v.union(v.literal('comment'), v.literal('reply'), v.literal('like'), v.literal('mention'), v.literal('new_follower'), v.literal('following_itinerary'), 
        ...
        referenceType: v.union(v.literal('itinerary'), v.literal('comment'), v.literal('user'), v.literal('flight'), v.literal('weather')),
    ...
    scheduledNotifications: defineTable({
        userId: v.string(),
        type: v.union(v.literal('itinerary_reminder'), v.literal('flight_checkin'), v.literal('flight_boarding'), v.literal('weather_check'), v.literal('custom')),
        ...
        status: v.union(v.literal('pending'), v.literal('sent'), v.literal('cancelled'), v.literal('failed')),
    ...
    messages: defineTable({
        ...
        content: v.string(),
        messageType: v.union(v.literal('text'), v.literal('image'), v.literal('itinerary_share')),
    ...
    hotelBookings: defineTable({
        ...
        importSource: v.optional(v.union(v.literal('manual'), v.literal('email'), v.literal('import'))),
        rawEmailContent: v.optional(v.string()), // Original email content if imported
        // Status
        status: v.optional(v.union(v.literal('confirmed'), v.literal('pending'), v.literal('cancelled'), v.literal('completed'))),
    })
        .index('by_user', ['userId'])
        .index('by_itinerary', ['itineraryId'])
    ...
    itineraryBudgets: defineTable({
        ...
        categoryBudgets: v.array(v.object({
    ...
    calendarSyncs: defineTable({
        ...
        syncStatus: v.union(v.literal('pending'), v.literal('synced'), v.literal('failed'), v.literal('deleted')),
    ...
    travelStats: defineTable({
        ...
        mostVisitedCity: v.optional(v.object({
            name: v.string(),
            count: v.number(),
        })),
        mostVisitedCountry: v.optional(v.object({
            name: v.string(),
            count: v.number(),
        })),
        ...
        nextGoalCity: v.optional(v.object({
            cityName: v.string(),
            countryCode: v.string(),
            countryName: v.string(),
            latitude: v.number(),
    ...
    insuranceProducts: defineTable({
        ...
        // Product type
        type: v.union(v.literal('comprehensive'), // 综合旅行险
        ...
        coverageDetails: v.array(v.object({
        ...
        // Risk level coverage
        riskLevelCoverage: v.array(v.union(v.literal('low'), // Safe destinations
    ...
    userInsurance: defineTable({
        ...
        // Insured persons
        insuredPersons: v.array(v.object({
            name: v.string(),
            idType: v.union(v.literal('id_card'), // 身份证
            v.literal('passport'), // 护照
            v.literal('other')),
            idNumber: v.string(),
            phone: v.optional(v.string()),
            relationship: v.union(v.literal('self'), // 本人
            v.literal('spouse'), // 配偶
            v.literal('child'), // 子女
            v.literal('parent'), // 父母
            v.literal('other')),
        ...
        // Status
        status: v.union(v.literal('pending'), // Waiting for coverage to start
        ...
        // Claim info
        claimHistory: v.optional(v.array(v.object({
            claimId: v.string(),
            claimDate: v.number(), // Unix timestamp
            claimType: v.string(),
            claimAmount: v.number(),
            status: v.union(v.literal('submitted'), v.literal('processing'), v.literal('approved'), v.literal('rejected'), v.literal('paid')),
            notes: v.optional(v.string()),
        }))),
        // Notes
        notes: v.optional(v.string()),
    ...
    destinationRiskProfiles: defineTable({
        ...
        riskLevel: v.union(v.literal('low'), v.literal('medium'), v.literal('high'), v.literal('extreme')),
        riskFactors: v.array(v.string()), // Risk factors description
        recommendedInsuranceTypes: v.array(v.union(v.literal('comprehensive'), v.literal('medical'), v.literal('accident'), v.literal('flight_delay'), v.literal('luggage'), v.literal('cancellation'), v.literal('emergency_evacuation'))),
    ...
    insuranceClaimGuides: defineTable({
        title: v.string(),
        claimType: v.union(v.literal('medical'), // 医疗理赔
    ...
    chatMessages: defineTable({
        ...
        metadata: v.optional(v.object({
            // POI recommendations
            pois: v.optional(v.array(v.object({
                name: v.string(),
                type: v.string(),
                description: v.optional(v.string()),
                latitude: v.optional(v.number()),
            ...
            itineraryChanges: v.optional(v.array(v.object({
    ...
    tippingGuides: defineTable({
        ...
        tippingCulture: v.union(v.literal('expected'), // 小费是社会规范，几乎必须
        ...
        scenarios: v.array(v.object({
            type: v.union(v.literal('restaurant'), // 餐厅
            v.literal('hotel'), // 酒店
            v.literal('taxi'), // 出租车
            v.literal('bar'), // 酒吧
            v.literal('spa'), // 水疗/按摩
    ...
    emergencyServices: defineTable({
        ...
        consulateInfo: v.optional(v.array(v.object({
            city: v.string(),
            phone: v.string(),
            address: v.optional(v.string()),
        }))), // List of consulates
    ...
    sosAlerts: defineTable({
        ...
        // Alert details
        alertType: v.union(v.literal('emergency'), v.literal('medical'), v.literal('safety'), v.literal('other')),
        message: v.optional(v.string()), // Optional user message
        // Status tracking
        status: v.union(v.literal('sent'), v.literal('received'), v.literal('resolved'), v.literal('cancelled')),
        // Notifications
    ...
    safetyAlerts: defineTable({
        ...
        // Alert details
        alertType: v.union(v.literal('travel_advisory'), // Government travel advisory
        ...
        v.literal('other')),
        // Severity levels
        severity: v.union(v.literal('info'), // Informational
    ...
    dangerZones: defineTable({
        ...
        dangerLevel: v.union(v.literal('caution'), // Be careful
        ...
        dangerTypes: v.array(v.union(v.literal('crime'), v.literal('scam'), v.literal('traffic'), v.literal('natural_hazard'), v.literal('political'), v.literal('health'), v.literal('other'))),
        // Description
        description: v.string(),
        descriptionEn: v.optional(v.string()),
    ...
    wifiSpots: defineTable({
        ...
        name: v.string(),
        nameEn: v.optional(v.string()),
        type: v.union(v.literal('hotel'), v.literal('restaurant'), v.literal('cafe'), v.literal('airport'), v.literal('train_station'), v.literal('shopping_mall'), v.literal('library'), v.literal('coworking'), v.literal('public'), v.literal('other')),
        // Location
        cityId: v.id('cities'),
        address: v.optional(v.string()),
        latitude: v.number(),
    ...
    wifiCredentials: defineTable({
        ...
        securityType: v.optional(v.union(v.literal('open'), v.literal('wep'), v.literal('wpa'), v.literal('wpa2'), v.literal('wpa3'), v.literal('unknown'))),
    ...
    safetyIncidentReports: defineTable({
        ...
        longitude: v.optional(v.number()),
        // Incident details
        incidentType: v.union(v.literal('theft'), v.literal('assault'), v.literal('scam'), v.literal('harassment'), v.literal('traffic_accident'), v.literal('natural_disaster'), v.literal('health_issue'), v.literal('police_issue'), v.literal('other')),
        severity: v.union(v.literal('minor'), // Inconvenience
    ...
    chargingStations: defineTable({
        ...
        longitude: v.number(),
        // Station Details
        stationType: v.union(v.literal('public'), // 公共充电站
        ...
        chargerTypes: v.array(v.object({
            type: v.union(v.literal('ac_slow'), // 交流慢充 (7kW)
        ...
        is24Hours: v.boolean(),
        // Amenities
        amenities: v.optional(v.array(v.union(v.literal('restroom'), // 卫生间
        ...
        // Status
        status: v.union(v.literal('operational'), // 正常运营
        ...
        updatedAt: v.optional(v.number()),
        // Payment Methods
        paymentMethods: v.optional(v.array(v.union(v.literal('app'), // APP支付
    ...
    simCards: defineTable({
        ...
        // Card Type
        cardType: v.union(v.literal('physical'), // 实体SIM卡
        v.literal('esim'), // eSIM
        ...
        physicalSimInfo: v.optional(v.object({
    ...
    simCardReviews: defineTable({
        ...
        signalQuality: v.optional(v.union(v.literal('excellent'), // 优秀
        v.literal('good'), // 良好
        ...
        // Moderation
        status: v.union(v.literal('pending'), // 待审核
    ...
    travelNotes: defineTable({
        ...
        title: v.string(),
        content: v.string(),
        visibility: v.union(v.literal('private'), v.literal('public'), v.literal('followers')),
    ...
    userTravelStats: defineTable({
        ...
        // Time-based stats
        longestTrip: v.optional(v.object({
            itineraryId: v.id('itineraries'),
            title: v.string(),
            days: v.number(),
            startDate: v.string(),
        ...
        shortestTrip: v.optional(v.object({
            itineraryId: v.id('itineraries'),
            title: v.string(),
            days: v.number(),
            startDate: v.string(),
        ...
        topDestinations: v.array(v.object({
            cityId: v.id('cities'),
            cityName: v.string(),
            visitCount: v.number(),
            totalDays: v.number(),
        ...
        // Travel patterns
        preferredTransportModes: v.array(v.object({
            mode: v.string(), // walking, driving, transit, etc.
            count: v.number(),
            percentage: v.number(),
        })),
        preferredPoiCategories: v.array(v.object({
            category: v.string(), // attraction, restaurant, hotel, etc.
            count: v.number(),
            percentage: v.number(),
        })),
        // Monthly distribution
        monthlyTripCounts: v.array(v.object({
            month: v.number(), // 1-12
            count: v.number(),
        })),
        // Timestamps
    ...
    yearlyReviews: defineTable({
        ...
        expenseBreakdown: v.array(v.object({
            categoryId: v.id('expenseCategories'),
            categoryName: v.string(),
        ...
        mostExpensiveTrip: v.optional(v.object({
            itineraryId: v.id('itineraries'),
            title: v.string(),
            amount: v.number(),
        })),
        // Highlights
        firstTripOfYear: v.optional(v.object({
            itineraryId: v.id('itineraries'),
            title: v.string(),
            cityName: v.string(),
            startDate: v.string(),
        })),
        lastTripOfYear: v.optional(v.object({
            itineraryId: v.id('itineraries'),
            title: v.string(),
            cityName: v.string(),
            startDate: v.string(),
        ...
        monthlyActivity: v.array(v.object({
            month: v.number(), // 1-12
            tripsCount: v.number(),
            daysCount: v.number(),
            expenses: v.number(),
        ...
        yearOverYear: v.optional(v.object({
        ...
        memories: v.optional(v.array(v.object({
            text: v.string(),
            itineraryId: v.optional(v.id('itineraries')),
            imageUrl: v.optional(v.string()),
            createdAt: v.number(),
    ...
    verificationBadges: defineTable({
        userId: v.string(), // Auth user ID
        badgeType: v.union(v.literal('travel_expert'), // 旅行达人认证
    ...
    verificationApplications: defineTable({
        ...
        badgeType: v.union(v.literal('travel_expert'), v.literal('local_guide'), v.literal('official_account')),
        // Application status
        status: v.union(v.literal('pending'), // 待审核
        ...
        supportingMaterials: v.optional(v.array(v.object({
            type: v.union(v.literal('id_photo'), // 证件照片
    ...
    packingLists: defineTable({
        ...
        tripType: v.optional(v.union(v.literal('leisure'), // 休闲
    ...
    packingItems: defineTable({
        ...
        category: v.union(v.literal('clothing'), // 衣物
        ...
        suggestedBy: v.optional(v.union(v.literal('user'), // User added
    ...
    packingTemplates: defineTable({
        ...
        tripType: v.union(v.literal('leisure'), v.literal('business'), v.literal('adventure'), v.literal('beach'), v.literal('ski'), v.literal('city'), v.literal('hiking'), v.literal('other')),
        climate: v.optional(v.union(v.literal('tropical'), // 热带
        v.literal('dry'), // 干燥
    ...
    sharedExpenses: defineTable({
        ...
        description: v.string(), // What was purchased
        category: v.union(v.literal('food'), // 餐饮
        ...
        splitType: v.union(v.literal('equal'), // Split equally among participants
    ...
    itineraryTemplates: defineTable({
        ...
        days: v.array(v.object({
            ...
            pois: v.array(v.object({
                name: v.string(),
                type: v.union(v.literal('attraction'), v.literal('restaurant'), v.literal('hotel'), v.literal('transportation'), v.literal('activity'), v.literal('shopping')),
                description: v.optional(v.string()),
        ...
        estimatedBudget: v.optional(v.object({
            min: v.number(),
            max: v.number(),
            currency: v.string(),
        })),
    ...
    itineraryDrafts: defineTable({
        ...
        days: v.optional(v.array(v.object({
            ...
            items: v.array(v.object({
                ...
                endTime: v.optional(v.string()),
                transportMode: v.optional(v.union(v.literal('walking'), v.literal('driving'), v.literal('transit'), v.literal('cycling'), v.literal('taxi'))),
                notes: v.optional(v.string()),
                // Inline POI data for unsaved POIs
                inlinePoi: v.optional(v.object({
                    name: v.string(),
                    category: v.union(v.literal('attraction'), v.literal('restaurant'), v.literal('hotel'), v.literal('shopping'), v.literal('other')),
                    address: v.optional(v.string()),
                    latitude: v.optional(v.number()),
    ...
    userTravelPreferences: defineTable({
        userId: v.string(), // Auth user ID
        travelStyles: v.optional(v.array(v.union(v.literal('adventure'), v.literal('relaxation'), v.literal('culture'), v.literal('food'), v.literal('nature'), v.literal('shopping'), v.literal('photography'), v.literal('budget'), v.literal('luxury')))),
        preferredPace: v.optional(v.union(v.literal('slow'), v.literal('moderate'), v.literal('fast'))),
        languages: v.optional(v.array(v.string())),
        ageRange: v.optional(v.union(v.literal('18-25'), v.literal('26-35'), v.literal('36-45'), v.literal('46-55'), v.literal('55+'))),
        gender: v.optional(v.union(v.literal('male'), v.literal('female'), v.literal('other'))),
        preferredPartnerGender: v.optional(v.union(v.literal('male'), v.literal('female'), v.literal('other'), v.literal('any'))),
        bio: v.optional(v.string()),
        interests: v.optional(v.array(v.string())),
        smokingPreference: v.optional(v.union(v.literal('smoker'), v.literal('non_smoker'), v.literal('no_preference'))),
        accommodationPreference: v.optional(v.union(v.literal('hostel'), v.literal('budget_hotel'), v.literal('mid_range'), v.literal('luxury'), v.literal('no_preference'))),
        createdAt: v.number(),
        updatedAt: v.number(),
    })
        .index('by_user', ['userId'])
    ...
    travelPartnerRequests: defineTable({
        ...
        preferredGender: v.optional(v.union(v.literal('male'), v.literal('female'), v.literal('other'), v.literal('any'))),
        preferredAgeRange: v.optional(v.array(v.union(v.literal('18-25'), v.literal('26-35'), v.literal('36-45'), v.literal('46-55'), v.literal('55+')))),
        travelStyles: v.optional(v.array(v.union(v.literal('adventure'), v.literal('relaxation'), v.literal('culture'), v.literal('food'), v.literal('nature'), v.literal('shopping'), v.literal('photography'), v.literal('budget'), v.literal('luxury')))),
        budgetRange: v.optional(v.union(v.literal('budget'), v.literal('moderate'), v.literal('comfortable'), v.literal('luxury'))),
        estimatedBudget: v.optional(v.number()),
        itineraryId: v.optional(v.id('itineraries')),
        coverImageUrl: v.optional(v.string()),
        imageUrls: v.optional(v.array(v.string())),
        status: v.union(v.literal('active'), v.literal('paused'), v.literal('fulfilled'), v.literal('cancelled'), v.literal('expired')),
        viewCount: v.number(),
    ...
    partnerMatchApplications: defineTable({
        ...
        matchFactors: v.optional(v.object({
        ...
        status: v.union(v.literal('pending'), v.literal('accepted'), v.literal('rejected'), v.literal('withdrawn'), v.literal('expired')),
    ...
    partnerMatches: defineTable({
        ...
        ownerFeedback: v.optional(v.object({
            rating: v.number(),
            review: v.optional(v.string()),
            wouldTravelAgain: v.boolean(),
            createdAt: v.number(),
        })),
        partnerFeedback: v.optional(v.object({
            rating: v.number(),
            review: v.optional(v.string()),
            wouldTravelAgain: v.boolean(),
            createdAt: v.number(),
    ...
    userVerifications: defineTable({
        userId: v.string(),
        verificationType: v.union(v.literal('identity'), v.literal('phone'), v.literal('email'), v.literal('social'), v.literal('travel_history'), v.literal('reference')),
    ...
    userTrustScores: defineTable({
        ...
        badges: v.optional(v.array(v.union(v.literal('verified_identity'), v.literal('trusted_traveler'), v.literal('super_host'), v.literal('responsive'), v.literal('experienced'), v.literal('top_rated')))),
    ...
    poiQuestions: defineTable({
        ...
        category: v.union(v.literal('general'), // 一般问题
        ...
        // Status
        status: v.union(v.literal('open'), // 开放中
    ...
    poiAnswers: defineTable({
        ...
        authorBadgeType: v.optional(v.union(v.literal('travel_expert'), v.literal('local_guide'), v.literal('official_account'))),
        // Moderation
        reportCount: v.number(),
        isHidden: v.boolean(),
        // Timestamps
    ...
    questionReports: defineTable({
        questionId: v.id('poiQuestions'),
        userId: v.string(), // Auth user ID
        reason: v.union(v.literal('spam'), v.literal('inappropriate'), v.literal('duplicate'), v.literal('off_topic'), v.literal('other')),
        description: v.optional(v.string()),
        status: v.union(v.literal('pending'), v.literal('reviewed'), v.literal('resolved'), v.literal('dismissed')),
        createdAt: v.number(),
        reviewedAt: v.optional(v.number()),
    ...
    answerReports: defineTable({
        answerId: v.id('poiAnswers'),
        userId: v.string(), // Auth user ID
        reason: v.union(v.literal('spam'), v.literal('inappropriate'), v.literal('misleading'), v.literal('plagiarism'), v.literal('other')),
        description: v.optional(v.string()),
        status: v.union(v.literal('pending'), v.literal('reviewed'), v.literal('resolved'), v.literal('dismissed')),
        createdAt: v.number(),
        reviewedAt: v.optional(v.number()),
    ...
    translationPhrases: defineTable({
        ...
        category: v.union(v.literal('greeting'), // 问候语
        v.literal('transportation'), // 交通
        v.literal('dining'), // 餐饮
        v.literal('shopping'), // 购物
        v.literal('accommodation'), // 住宿
        ...
        audioUrls: v.optional(v.array(v.object({
            lang: v.string(),
            url: v.string(),
        }))),
    ...
    savedTranslations: defineTable({
        ...
        targetLang: v.string(),
        // Translation type
        translationType: v.union(v.literal('text'), // 文字翻译
    ...
    poiPhotos: defineTable({
        ...
        // Photo category
        category: v.optional(v.union(v.literal('interior'), // Interior/indoor photos
    ...
    flights: defineTable({
        ...
        status: v.union(v.literal('scheduled'), v.literal('delayed'), v.literal('boarding'), v.literal('departed'), v.literal('in_air'), v.literal('landed'), v.literal('arrived'), v.literal('cancelled'), v.literal('diverted')),
    ...
    flightBookings: defineTable({
        ...
        cabinClass: v.union(v.literal('economy'), v.literal('premium_economy'), v.literal('business'), v.literal('first')),
        status: v.union(v.literal('confirmed'), v.literal('pending'), v.literal('cancelled'), v.literal('checked_in'), v.literal('boarded'), v.literal('completed')),
    ...
    visaRequirements: defineTable({
        ...
        // Visa type
        visaType: v.union(v.literal('visa_free'), // 免签
        ...
        // Difficulty level
        difficultyLevel: v.union(v.literal('very_easy'), // 非常容易（免签/落地签）
        ...
        entryType: v.optional(v.union(v.literal('single'), v.literal('multiple'), v.literal('dual'))), // 单次/多次入境
        ...
        applicationMethods: v.array(v.object({
            method: v.union(v.literal('online'), // 在线申请
    ...
    userVisaReminders: defineTable({
        ...
        // Visa info
        visaType: v.union(v.literal('visa_free'), v.literal('visa_on_arrival'), v.literal('e_visa'), v.literal('standard_visa'), v.literal('transit_visa'), v.literal('work_visa'), v.literal('student_visa'), v.literal('business_visa')),
        // Status
        status: v.union(v.literal('pending'), // 待提醒
        v.literal('sent'), // 已发送
    ...
    visaApplications: defineTable({
        ...
        // Status
        status: v.union(v.literal('preparing'), // 准备中
    ...
    visaCenters: defineTable({
        ...
        name: v.string(), // 中心名称
        nameEn: v.optional(v.string()),
        type: v.union(v.literal('embassy'), // 大使馆
        v.literal('consulate'), // 领事馆
        v.literal('visa_center'), // 签证中心
    ...
    shareLinks: defineTable({
        ...
        resourceType: v.union(v.literal('itinerary'), v.literal('travelGuide'), v.literal('travelNote')),
        resourceId: v.string(),
        ...
        platform: v.union(v.literal('wechat'), v.literal('weibo'), v.literal('xiaohongshu'), v.literal('qq'), v.literal('douyin'), v.literal('copy_link'), v.literal('system_share'), v.literal('generic')),
        // Access control
        permission: v.union(v.literal('public'), // Anyone with link can view
    ...
    shareEvents: defineTable({
        ...
        // Share details
        platform: v.union(v.literal('wechat'), v.literal('weibo'), v.literal('xiaohongshu'), v.literal('qq'), v.literal('douyin'), v.literal('copy_link'), v.literal('system_share'), v.literal('generic')),
        eventType: v.union(v.literal('share'), // Initial share action
    ...
    shareEventLogs: defineTable({
        ...
        // Event details
        platform: v.union(v.literal('wechat'), v.literal('weibo'), v.literal('xiaohongshu'), v.literal('qq'), v.literal('douyin'), v.literal('copy_link'), v.literal('system_share'), v.literal('generic')),
        eventType: v.union(v.literal('share'), v.literal('click'), v.literal('view'), v.literal('save')),
    ...
    luggage: defineTable({
        ...
        size: v.optional(v.union(v.literal('cabin'), // 登机箱
        v.literal('medium'), // 中型
        v.literal('large'), // 大型
        ...
        // Status tracking
        status: v.union(v.literal('checked_in'), // 已托运
    ...
    localEvents: defineTable({
        ...
        // Event Type
        eventType: v.union(v.literal('festival'), // 节日 (春节, 中秋等)
        ...
        recurrencePattern: v.optional(v.object({
            type: v.union(v.literal('yearly'), // 每年
        ...
        // Status
        status: v.union(v.literal('upcoming'), // 即将开始
    ...
    eventReminders: defineTable({
        ...
        eventId: v.id('localEvents'),
        // Reminder Settings
        reminderType: v.union(v.literal('event_start'), // 活动开始提醒
    ...
    userPreferences: defineTable({
        ...
        explicitPreferences: v.array(v.union(v.literal('food'), v.literal('culture'), v.literal('nature'), v.literal('shopping'), v.literal('nightlife'), v.literal('adventure'), v.literal('relaxation'), v.literal('photography'), v.literal('family'), v.literal('budget'), v.literal('luxury'))),
        // Travel style preferences
        travelStyle: v.union(v.literal('adventurous'), v.literal('relaxed'), v.literal('cultural'), v.literal('balanced')),
        // Budget preference
        budgetLevel: v.union(v.literal('budget'), v.literal('moderate'), v.literal('luxury')),
        // Pace preference
        pacePreference: v.union(v.literal('slow'), v.literal('moderate'), v.literal('fast')),
    ...
    userBehaviorEvents: defineTable({
        userId: v.string(), // Auth user ID
        // Behavior type
        behaviorType: v.union(v.literal('view'), v.literal('save'), v.literal('unsave'), v.literal('copy'), v.literal('share'), v.literal('like'), v.literal('unlike'), v.literal('search'), v.literal('poi_click'), v.literal('poi_add')),
        // Target information
        targetType: v.union(v.literal('guide'), v.literal('itinerary'), v.literal('poi'), v.literal('city'), v.literal('search')),
        targetId: v.string(),
        // Associated categories
        categories: v.array(v.union(v.literal('food'), v.literal('culture'), v.literal('nature'), v.literal('shopping'), v.literal('nightlife'), v.literal('adventure'), v.literal('relaxation'), v.literal('photography'), v.literal('family'), v.literal('budget'), v.literal('luxury'))),
        // Additional metadata
    ...
    itineraryVersions: defineTable({
        ...
        changesCount: v.optional(v.object({
            daysAdded: v.number(),
    ...
    poiQuestionsExtended: defineTable({
        ...
        // Status
        status: v.union(v.literal('open'), // Accepting answers
    ...
    poiQAReports: defineTable({
        ...
        // Report details
        reason: v.union(v.literal('spam'), v.literal('inappropriate'), v.literal('misleading'), v.literal('off_topic'), v.literal('harassment'), v.literal('other')),
        description: v.optional(v.string()),
        // Status
        status: v.union(v.literal('pending'), v.literal('reviewed'), v.literal('dismissed'), v.literal('actioned')),
        reviewedBy: v.optional(v.string()), // Admin user ID
        reviewedAt: v.optional(v.number()),
    ...
    agentSessions: defineTable({
        ...
        sessionType: v.union(v.literal('chat'), // General chat
        v.literal('travel_plan'), // Travel planning session
        v.literal('enrichment') // Content enrichment task
        ),
        status: v.union(v.literal('active'), v.literal('paused'), // Waiting for user input (interrupt)
        v.literal('completed'), v.literal('expired')),
        messages: v.array(v.object({
            role: v.union(v.literal('human'), v.literal('ai'), v.literal('system'), v.literal('tool')),
            content: v.string(),
