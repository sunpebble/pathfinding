...
const reminderTypeValidator = v.union(v.literal('opening'), v.literal('closing'), v.literal('best_time'));
// ============================================
// Helper Functions
// ============================================
/**
 ...
 */
function isWithinTimeSlot(currentMinutes, slot) {
...
export const getPoiWithBusinessHours = query({
    ...
    handler: async (ctx, args) => {
        ...
        const businessHours = poi.businessHours;
        ...
        const holidayHours = await ctx.db
            ...
            .filter((q) => q.and(q.lte(q.field('startDate'), today), q.gte(q.field('endDate'), today)))
            .first();
...
export const getPendingReminders = query({
    ...
    handler: async (ctx, args) => {
        return await ctx.db
            ...
            .filter((q) => q.and(q.lte(q.field('scheduledTime'), args.beforeTime), q.eq(q.field('isTriggered'), false)))
            .collect();
    },
});
/**
