...
const operationTypeValidator = v.union(v.literal('create'), v.literal('update'), v.literal('delete'), v.literal('reorder'));
const targetTypeValidator = v.union(v.literal('itinerary'), v.literal('day'), v.literal('item'));
const _statusValidator = v.union(v.literal('pending'), v.literal('applied'), v.literal('conflicted'), v.literal('rejected'));
const resolutionValidator = v.union(v.literal('accept_mine'), v.literal('accept_theirs'), v.literal('merge'));
/**
 * Get the current version for a target
 */
async function getCurrentVersion(ctx, itineraryId, targetType, targetId) {
    const lastOperation = await ctx.db
        .query('editOperations')
        .withIndex('by_itinerary_timestamp', (q) => q.eq('itineraryId', itineraryId))
        .order('desc')
        .filter((q) => q.and(q.eq(q.field('targetType'), targetType), q.eq(q.field('targetId'), targetId), q.eq(q.field('status'), 'applied')))
        .first();
 ...
 */
async function checkForConflicts(ctx, itineraryId, targetType, targetId, baseVersion, excludeUserId) {
    const conflictingOps = await ctx.db
        .query('editOperations')
        .withIndex('by_itinerary_timestamp', (q) => q.eq('itineraryId', itineraryId))
        .filter((q) => q.and(q.eq(q.field('targetType'), targetType), q.eq(q.field('targetId'), targetId), q.gte(q.field('version'), baseVersion), q.neq(q.field('userId'), excludeUserId), q.eq(q.field('status'), 'applied')))
        .collect();
...
export const getRecentOperations = query({
    ...
    handler: async (ctx, args) => {
        ...
        const operations = await ctx.db
            .query('editOperations')
            .withIndex('by_itinerary_timestamp', (q) => q.eq('itineraryId', args.itineraryId))
            .order('desc')
            .take(limit);
        return operations;
    },
...
export const getPendingOperations = query({
    ...
    handler: async (ctx, args) => {
        return await ctx.db
            .query('editOperations')
            .withIndex('by_itinerary_status', (q) => q.eq('itineraryId', args.itineraryId).eq('status', 'pending'))
            .collect();
    },
});
/**
...
export const getConflictedOperations = query({
    ...
    handler: async (ctx, args) => {
        return await ctx.db
            .query('editOperations')
            .withIndex('by_itinerary_status', (q) => q.eq('itineraryId', args.itineraryId).eq('status', 'conflicted'))
            .collect();
    },
});
/**
...
export const recordOperation = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // Check for conflicts
        const hasConflict = await checkForConflicts(ctx, args.itineraryId, args.targetType, args.targetId, baseVersion, args.userId);
        // Get the next version
        const nextVersion = await getCurrentVersion(ctx, args.itineraryId, args.targetType, args.targetId);
...
export const applyOperation = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (operation.status !== 'pending') {
            throw new Error(`Cannot apply operation with status: ${operation.status}`);
        }
...
export const resolveConflict = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const now = Date.now();
        const newStatus = args.resolution === 'accept_theirs' ? 'rejected' : 'applied';
        await ctx.db.patch(args.operationId, {
            status: newStatus,
            conflictResolution: {
