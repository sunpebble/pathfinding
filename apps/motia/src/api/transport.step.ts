...
const poiSchema = z.object({ name: z.string().min(1), latitude: z.number(), longitude: z.number() })
...
export async function handler(req: { body?: unknown }, { logger }: HandlerContext) {
  ...
  if (!parseResult.success) return { status: 400, body: { error: parseResult.error.message } }

  const { pois, transportMode } = parseResult.data
  const speeds: Record<TransportMode, number> = { walking: 5, driving: 40, transit: 25 }
  ...
  const segments = optimizedOrder.slice(0, -1).map((idx, i) => {
    const from = pois[idx], to = pois[optimizedOrder[i + 1]]
  ...
  return { status: 200, body: { optimizedOrder, segments, savings: { distanceKm: 0, durationMinutes: 0 } } }
}
