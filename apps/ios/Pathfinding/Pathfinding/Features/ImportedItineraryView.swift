...
=======
  ...
  @State private var selectedPoiIndex: Int? = nil
  @Environment(\.dismiss) private var dismiss
  @Environment(AppState.self) private var appState
...
<<<<<<< Updated upstream
      ...
      poiListView
        .frame(height: 220)
=======
      // Day picker with auto-scroll to selected
      ...
      Map(position: $cameraPosition) {
        ForEach(Array(annotations.enumerated()), id: \.element.id) { index, annotation in
          let isSelected = selectedPoiIndex == index
          Annotation(annotation.poi.name, coordinate: annotation.coordinate) {
            ZStack {
              Circle()
                ...
                .shadow(color: isSelected ? .red.opacity(0.5) : .clear, radius: 8)
              Text("\(index + 1)")
        ...
        if let day = currentDay {
          ForEach(Array(day.pois.enumerated()), id: \.offset) { index, poi in
            ...
            HStack(spacing: 12) {
              ZStack {
                Circle()
                  .fill(isSelected ? Color.red : colorForType(poi.type))
              ...
              Spacer()
              if poi.latitude != nil && poi.latitude != 0 {
                Image(systemName: "location.fill").foregroundStyle(isSelected ? .red : .green)
              }
            }
            .padding(.vertical, 4)
            ...
            .onTapGesture {
              withAnimation {
                if selectedPoiIndex == index {
                  selectedPoiIndex = nil
                  updateCamera()
                } else {
                  selectedPoiIndex = index
                  // Move camera to selected POI
                  if let lat = poi.latitude, let lng = poi.longitude, lat != 0 && lng != 0 {
      ...
      Button {
        ...
        Label(isSaved ? "已保存" : "保存到我的行程", systemImage: isSaved ? "checkmark.circle.fill" : "square.and.arrow.down")
          .frame(maxWidth: .infinity)
      }
      .buttonStyle(.borderedProminent)
      .disabled(isSaved)
...
=======
    .alert("保存成功", isPresented: $showingSaveConfirmation) {
      Button("查看行程") {
        appState.selectedTab = 2 // Switch to Itinerary Tab
        dismiss()
      }
      Button("继续浏览", role: .cancel) {}
    } message: {
