...
struct BlogListView: View {
  ...
  var filteredGuides: [BlogPost] {
    searchText.isEmpty
      ? guides : guides.filter { $0.title.localizedCaseInsensitiveContains(searchText) }
  }
        ...
        NavigationLink(value: guide) {
          HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
              Text(guide.title).font(.subheadline).fontWeight(.medium).lineLimit(2)
              HStack {
                if let author = guide.author { Text(author) }
                Spacer()
                if guide.aiProcessedAt != nil {
                  Label("AI", systemImage: "sparkles").foregroundStyle(.purple)
                }
              }
  ...
  private func loadGuides() async {
    isLoading = true
    do { guides = try await APIClient.shared.fetchGuides(limit: 50) } catch {
      print("Failed: \(error)")
    }
    isLoading = false
  }
