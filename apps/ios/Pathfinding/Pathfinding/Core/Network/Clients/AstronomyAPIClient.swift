...
extension APIClient {
  ...
  func fetchSunTimes(
    ...
    try await AstronomyAPIClient.shared.fetchSunTimes(latitude: latitude, longitude: longitude, date: date, timezone: timezone)
  }
  ...
  func fetchSunTimesRange(
    ...
    try await AstronomyAPIClient.shared.fetchSunTimesRange(latitude: latitude, longitude: longitude, startDate: startDate, endDate: endDate, timezone: timezone)
  }

  /// Fetch moon phase for a specific date
  func fetchMoonPhase(date: String? = nil) async throws -> MoonPhase {
    try await AstronomyAPIClient.shared.fetchMoonPhase(date: date)
  }

  /// Fetch moon phases for a date range
  func fetchMoonPhases(startDate: String, endDate: String) async throws -> [MoonPhase] {
    try await AstronomyAPIClient.shared.fetchMoonPhases(startDate: startDate, endDate: endDate)
  }
  ...
  func fetchAstronomicalEvents(
    ...
    try await AstronomyAPIClient.shared.fetchAstronomicalEvents(startDate: startDate, endDate: endDate, types: types)
  }
  ...
  func fetchStargazingSpots(
    ...
    try await AstronomyAPIClient.shared.fetchStargazingSpots(latitude: latitude, longitude: longitude, radiusKm: radiusKm, limit: limit)
  }
  ...
  func fetchAstronomyData(
    ...
    try await AstronomyAPIClient.shared.fetchAstronomyData(latitude: latitude, longitude: longitude, date: date, timezone: timezone, includeEvents: includeEvents, includeSpots: includeSpots)
  }
}
