            ...
            <div>
              ...
              <ul className="space-y-1.5">
                {(guide.aiTips || guide.ai_tips).map((tip: string, i: number) => (
                  <li key={`${i}-${tip.slice(0, 10)}`} className="flex items-start gap-2 text-sm text-gray-600">
            ...
            ? (
                ...
                <div className="prose prose-gray max-w-none">
                  {guide.content.split(/\n{2,}/).map((paragraph: string, i: number) => (
                    <p key={`${i}-${paragraph.slice(0, 10)}`} className="text-gray-700 leading-relaxed mb-4">
                      {paragraph.trim()}
                    </p>
                  ))}
                </div>
