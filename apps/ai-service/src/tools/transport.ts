...
function extractSteps(path: AmapPath, mode: TransportMode): string[] {
  if (mode === "transit") {
    const segments = path.segments || [];
    // eslint-disable-next-line ts/no-explicit-any -- AmapSegment has complex nested structure
  ...
  return (
    steps
      .slice(0, 5)
      // eslint-disable-next-line ts/no-explicit-any -- AmapStep has dynamic instruction/action fields
      .map((step: any) => step.instruction || step.action || "")
  );
}
