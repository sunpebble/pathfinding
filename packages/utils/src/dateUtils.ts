 ...
 */
export function combineDateAndTime(date: Date | string, timeStr: string, timezone?: string): Date {
  const d = typeof date === "string" ? new Date(date) : date;
  ...
  if (timezone) {
    ...
    const utcDate = new Date(result.toLocaleString("en-US", { timeZone: "UTC" }));
 ...
 */
export function getRelativeTime(date: Date | string, locale: "zh" | "en" = "zh"): string {
  const d = typeof date === "string" ? new Date(date) : date;
  const now = new Date();
