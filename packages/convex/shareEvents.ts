...
export const getStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    let totalShares = shareEvents.filter((e) => e.eventType === 'share').length;
    let totalClicks = 0;
    let totalViews = 0;
    let totalSaves = 0;
...
export const getTopShared = query({
  ...
  handler: async (ctx, args) => {
    ...
    const sharesByResource = new Map<string, { count: number; resourceType: string }>();

    for (const event of events) {
      if (event.eventType === 'share') {
        const key = `${event.resourceType}:${event.resourceId}`;
        const current = sharesByResource.get(key) || { count: 0, resourceType: event.resourceType };
