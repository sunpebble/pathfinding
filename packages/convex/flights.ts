  ...
  handler: async (ctx, args) => {
    const flight = await ctx.db
      ...
      .withIndex('by_flight_number_date', (q) =>
        q.eq('flightNumber', args.flightNumber.toUpperCase()).eq('departureDate', args.date)
      )
      .first();
  ...
  handler: async (ctx, args) => {
    ...
    let flights = await ctx.db
      ...
      .withIndex('by_route', (q) =>
        q.eq('departureAirport', args.departureAirport.toUpperCase())
...
export const upsert = mutation({
  ...
  handler: async (ctx, args) => {
    const existing = await ctx.db
      .query('flights')
      .withIndex('by_flight_number_date', (q) =>
        q.eq('flightNumber', args.flightNumber.toUpperCase()).eq('departureDate', args.departureDate)
      )
      .first();
