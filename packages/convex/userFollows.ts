...
export const getFollowRecommendations = query({
  ...
  handler: async (ctx, args) => {
    ...
    const friendsOfFriends: Map<string, { count: number; source: string }> = new Map();
      ...
      for (const ff of theirFollowing) {
        if (!followingIds.has(ff.followingId)) {
          ...
          if (existing) {
            ...
            friendsOfFriends.set(ff.followingId, { count: 1, source: 'friends_of_friends' });
          }
        }
      }
    }
    ...
    for (const profile of popularUsers) {
      const existing = recommendations.get(profile.email) ?? { score: 0, reasons: [] };
...
export const batchCheckFollowStatus = query({
  ...
  handler: async (ctx, args) => {
    const results: Record<string, { isFollowing: boolean; isFollowedBy: boolean; isMutual: boolean }> = {};
