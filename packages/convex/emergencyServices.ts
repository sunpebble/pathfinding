    ...
    const matches = services.filter(
      (s) =>
        ...
        (s.countryName.toLowerCase().includes(searchTerm) ||
          (s.countryNameEn && s.countryNameEn.toLowerCase().includes(searchTerm)))
    );
  ...
  handler: async (ctx) => {
    ...
    for (const country of countries) {
      ...
      if (!existing) {
        ...
        results.push({ countryCode: country.countryCode, id, action: 'created' });
      } else {
        results.push({
...
export const getEmergencyGuide = query({
  ...
  handler: async (ctx, args) => {
    ...
    const guides: Record<string, { title: string; titleEn: string; steps: string[]; tips: string[] }> = {
      medical: {
        title: '医疗紧急情况',
