...
describe('likes routes', () => {
  ...
  it('gET /api/likes returns paginated liked itineraries', async () => {
    // Items query chain
    const itemsOffset = vi.fn().mockResolvedValue([
      { itinerary_likes: { id: 1, userId: 1, itineraryId: 5 }, itineraries: { id: 5, title: 'Kyoto' } },
    ]);
    ...
    const response = await requestWithAuth(createApp(), '/api/likes');
  ...
  it('gET /api/likes requires auth', async () => {
    const response = await requestWithEnv(createApp(), '/api/likes');
