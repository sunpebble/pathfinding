...
export const crawlJobs = mysqlTable('crawl_jobs', {
  ...
  createdAt: createdAt(),
  updatedAt: updatedAt(),
}, t => [index('crawl_jobs_status_idx').on(t.status), index('crawl_jobs_platform_idx').on(t.platform)]);

export const rawCrawlRecords = mysqlTable('raw_crawl_records', {
  id: id(),
...
export const dataQualityReports = mysqlTable('data_quality_reports', {
  ...
  createdAt: createdAt(),
}, t => [index('quality_reports_dataset_idx').on(t.datasetId)]);

export const travelBlogPosts = mysqlTable('travel_blog_posts', {
  id: id(),
  platform: varchar('platform', { length: 50 }).notNull(),
