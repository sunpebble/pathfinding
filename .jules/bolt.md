...
**Action:** Always check for or create a lightweight auxiliary table (containing only the ID and the filterable field) to scan instead. Fetch full documents by ID only _after_ filtering. This reduced memory pressure significantly for `getByDestination`.
