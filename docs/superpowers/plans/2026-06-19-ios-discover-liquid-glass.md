...
Run: _BUILD_ Expected: exit 0(`xcodegen generate` 自动从工程移除该源)。
...
Run: _TEST SearchViewQueryTests_ Expected: FAIL —— `SearchView`/`makeSearchParams` 未定义。
...
Run: _TEST SearchViewQueryTests_ → PASS;_BUILD_ → exit 0。运行:确认尾部出现搜索 Tab、Discover 不再有内联搜索、搜索结果可点进详情。
...
Run: _BUILD_ → exit 0。预览三态 + 三档 a11y。确认无装饰背景、精选卡为玻璃且同簇一个 Container、行无显式玻璃。
...
Run: _TEST CityEncyclopediaReachabilityTests_ → FAIL（`hotCities` 未定义）。
...
Run: _TEST_ → PASS;_BUILD_ → exit 0。运行:Discover 出现热门城市,点击进入 CityEncyclopedia(此时仍为旧样式,T8 再玻璃化),数据真实加载。
...
Run: _BUILD_ → exit 0。为 `BlogDetailView` 加 `#Preview`(用示例 `BlogPost`)。手动:从搜索/Discover 进入详情,媒体切换、文章展开、地图、POI、import、评论与重构前一致。
...
Run: _BUILD_ → exit 0。预览各 section 三态 + 三档 a11y。手动:卡片玻璃、CTA、HUD、文章只一套折叠、目的地城市可进百科。
...
Run: _BUILD_ → exit 0。手动(经 T4 入口进入):四 tab 切换、内容与重构前一致;Picker 为系统分段控件。
...
Run: _BUILD_ → exit 0。预览四 tab + 加载/空/错三态 + 三档 a11y。手动确认无叠玻璃、急救按钮可拨号且有玻璃反馈。
...
Run: _BUILD_ → exit 0。全量测试:`cd apps/ios/Pathfinding && xcodebuild test -scheme Pathfinding-Debug -destination "platform=iOS Simulator,id=<SIMID>" -only-testing:PathfindingTests -quiet` → 全过。手动回归(三档 a11y):Discover 浏览 → 搜索 Tab → 指南详情 → 目的地城市/热门城市百科。
